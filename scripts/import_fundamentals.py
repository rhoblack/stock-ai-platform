"""Operator CLI for importing FundamentalSnapshot CSV rows.
...
--------
    ...
    python -m scripts.import_fundamentals --file fundamentals.csv
    python -m scripts.import_fundamentals --file fundamentals.csv --commit
    python -m scripts.import_fundamentals --file fundamentals.csv --encoding cp949
    python -m scripts.import_fundamentals --file fundamentals.csv --db-url sqlite:///./trial.db --commit
"""
...
def run_import(
    *,
    ...
    session = factory()
    try:
        importer = FundamentalCsvImporter(session)
...
def _print_summary(summary: ImportSummary, *, commit: bool, file_path: str) -> None:
    ...
    print(f"Fundamental import - {mode}")
