...
- [ ] `app/broker/fill_sync_service.py` 갱신 — 실 transport 호출 + 5 분기 + 델타 기반 idempotent 업데이트:
      `existing_total = sum(fills.fill_quantity)` / `kis_total = response.filled_qty` /
...
- [ ] 5 상태 분류: FULL / PARTIAL / NONE / REJECTED / CANCELED → `RealOrder.status` 갱신
- [ ] dry-run RealOrder 는 transport 호출 없이 NONE 반환 (v0.16 동작 유지)
- [ ] `app/api/real_order_routes.py` 신규 mutation — `POST /api/real-orders/{id}/sync`
      (KillSwitch + TradingSafety + AUTH 3중 게이트, v0.15 패턴)
- [ ] `app/api/schemas.py` — `RealOrderSyncResponse` 신규 (`real_order_status` / `fills_added` /
      `fills_total` / `synced_at`). forbidden 응답 필드 0건
- [ ] `tests/unit/test_fill_sync_service.py` v1.0 케이스 ~25 건 추가:
      FULL / PARTIAL / NONE / REJECTED / CANCELED / 반복 호출 idempotent (delta=0) / delta<0 raise / dry-run skip
- [ ] `tests/integration/test_fill_sync_integration.py` (신규) `respx` mock + DB 통합 ~10 건
- [ ] `tests/integration/test_real_order_sync_api.py` (신규) — `POST /sync` ~10 건:
      503 KillSwitch / 503 TradingSafety / 401 AUTH / 404 / 200 happy / 409 dry-run / 405 PUT/DELETE
- [ ] `tests/integration/test_auth_security.py` — mutating endpoint count 갱신 (현재 +1)
- [ ] `tests/integration/test_alembic_migration.py` — `HEAD_REVISION='0010_real_fills'` 그대로 (변경 0건)
- [ ] AST 가드: `fill_sync_service.py` 의 직접 `httpx.Client` 0건
- [ ] pytest `~1980 → ~2025` (+45) / 회귀 0건 / Alembic 0건 / 프런트 0건
