...
## v0.16 — Real Order Integration Skeleton & Fill Sync Readiness

세부 계획: [`PLANS.md`](./PLANS.md) `PLAN-0016`. 채택 시나리오: **Scenario X (핵심) +
Scenario Y Phase 1 (RealOrder/RealFill ORM)**. 기준선 `v0.15-final` (pytest 1693 /
vitest 201 / e2e 23 / build 그린). Alembic head: `0008_approval_audit_logs` → 목표
`0010_real_fills`.

### Phase A — RealTradingSettings + safety defaults

- [x] `app/config/settings.py`: 5 종 신규 필드 추가
      (`real_trading_enabled=False` / `kis_order_enabled=False` / `real_order_dry_run=True` /
      `max_real_order_amount=100_000` / `max_real_daily_order_amount=1_000_000`)
- [x] env override 5 종: `REAL_TRADING_ENABLED` / `KIS_ORDER_ENABLED` / `REAL_ORDER_DRY_RUN` /
      `MAX_REAL_ORDER_AMOUNT` / `MAX_REAL_DAILY_ORDER_AMOUNT`
- [x] `__post_init__` 검증: `max_real_order_amount > 0` / `max_real_daily_order_amount > 0` /
      `max_real_order_amount ≤ max_real_daily_order_amount`
- [x] `can_attempt_real_order_settings(settings) -> bool` 순수 판단 함수 추가 (6 gate AND)
- [x] `tests/unit/test_safety_settings.py` v0.16 신규 단위 테스트 60 건 추가
      (paranoid defaults / env override / 경계 검증 / 상호 의존 / typo 안전 default /
      can_attempt helper / Phase A scope 가드 / v0.15 회귀 유지)
- [x] `tests/unit/test_project_structure.py` 5 신규 default 단언 추가
- [x] pytest **1693 → 1753 passed (+60)** / 회귀 0건 / Alembic revision 0건 /
      DB 모델 변경 0건 / API 라우터 변경 0건 / 프런트 변경 0건
- [x] `tag v0.16-real-trading-settings + push`

### Phase B — KIS Order Wrapper Skeleton + FakeKisOrderTransport

- [x] `app/broker/kis_order_client.py` (신규):
      ...
      `KisOrderRequest` / `KisOrderResult` / `KisFillStatusResult` / `KisCancelResult` dataclass +
      `mask_sensitive_order_payload()` helper
- [x] `app/broker/__init__.py`: `KisOrderClientInterface` / `FakeKisOrderTransport` export 갱신
- [x] `tests/unit/test_kis_order_client.py` (신규) 35 건:
      ABC 직접 인스턴스화 불가 / 불변성 / repr·as_dict 마스킹 / 입력 검증 /
      FillStatus 유효 status / FakeKisOrderTransport mock 동작 /
      mask_sensitive_order_payload / AST 가드 4종 / Phase C·D scope 가드 2건
- [x] `tests/unit/test_safety_settings.py` Phase A guard 제거 (kis_order_client.py 존재 허용)
- [x] pytest **1753 → 1787 passed (+34)** / 회귀 0건 / Alembic revision 0건 / DB 모델 0건 / API 라우터 0건
- [x] `tag v0.16-kis-order-wrapper + push`

### Phase C — RealOrder / RealFill ORM + Alembic

- [x] `app/db/models.py`: `RealOrder` (40 번째 ORM) + `RealFill` (41 번째 ORM) 추가
      (`dry_run=True` 기본 / `broker_order_no_hash` SHA-256 only /
      FORBIDDEN 컬럼: `api_key` / `secret` / `account_number` / `raw_response` / `kis_response_raw`)
- [x] `alembic/versions/0009_real_orders.py` (신규): 1 테이블 + 5 인덱스 + FK + downgrade
- [x] `alembic/versions/0010_real_fills.py` (신규): 1 테이블 + 4 인덱스 + FK + downgrade
- [x] `app/data/repositories/real_order.py` (신규): `RealOrderRepository`
      (`create` / `get_by_id` / `get_by_candidate_id` / `list_recent` / `list_by_status` /
      `update_status` / `mark_submitted` / `mark_failed` / `mark_dry_run`)
- [x] `app/data/repositories/real_fill.py` (신규): `RealFillRepository`
      (`create` / `list_by_order` / `list_recent`)
- [x] `app/data/repositories/__init__.py`: `RealOrderRepository` / `RealFillRepository` export 추가
- [x] `tests/integration/test_real_order_repository.py` (신규) 48 건:
      create validation / forbidden 컬럼 11 종 / status 전이 / mark_failed 민감값 거부 /
      cascade delete / AST 가드 3 종 / Phase D scope 가드
- [x] `tests/integration/test_real_fill_repository.py` (신규) 23 건:
      create validation / list / cascade delete / forbidden 컬럼 11 종 / AST 가드 2 종 / raw 메서드 부재
- [x] `tests/integration/test_alembic_migration.py`:
      `HEAD_REVISION='0010_real_fills'` / `EXPECTED_TABLE_COUNT=41` / spot-check 2 건 추가
- [x] `tests/unit/test_safety_settings.py` / `test_kis_order_client.py`:
      Phase A/B scope guard 갱신 (real_order.py 존재 허용, revision 10개 확인)
- [x] pytest **1787 → 1858 passed (+71)** / 회귀 0건 / Alembic head `0010_real_fills` /
      누적 테이블 41 개 / compare_metadata diff 0건
- [x] `tag v0.16-real-order-orm + push`

### Phase D — RealOrderExecutor dry-run + FillSyncMock

- [x] `app/broker/real_order_executor.py` (신규):
      `RealOrderExecutor` + `ExecutorResult` dataclass (8-gate):
      1. `candidate.status == APPROVED` 검사 2. duplicate guard 3. `kill_switch_enabled`
      4. `real_trading_enabled` 5. `kis_order_enabled` 6. per-order cap
      7. daily cumulative cap 8. `PreTradeRiskEngine.evaluate()` 재검사
      → `FakeKisOrderTransport` → `RealOrder(dry_run=True, status=DRY_RUN)` 저장
- [x] `app/broker/fill_sync_service.py` (신규): `FillSyncService` mock
      (`sync_fills(session, real_order_id)` → FillSyncResult, FULL/PARTIAL/NONE 분기,
      transport 주입 가능, DRY_RUN 주문 fill 생성 but 상태 미갱신)
- [x] `app/broker/__init__.py`: ExecutorResult / FillSyncResult / FillSyncService /
      RealOrderExecutor 4건 export 추가
- [x] `tests/unit/test_real_order_executor.py` (신규) 26건:
      ExecutorResult frozen/as_dict / gate 1~8 차단 / dry-run 성공 / non-dry-run 차단 /
      FakeTransport only / AST 가드 4종 / no-raw-method / Phase E scope guard
- [x] `tests/unit/test_fill_sync_service.py` (신규) 21건:
      FillSyncResult frozen/as_dict / FULL fill / PARTIAL fill / PENDING→NONE /
      terminal order skip / NOT_FOUND / transport 주입 / AST 가드 4종 / amount positive
- [x] pytest **1858 → 1905 passed (+47)** / 회귀 0건 / Alembic revision 0건 / API 라우터 0건 / 프런트 0건
- [x] `tag v0.16-real-order-executor + push`

### Phase E — Real Trading UI (read-only) + 문서 마감

백엔드 (read-only API):
- [x] `app/api/real_order_routes.py` (신규):
      `GET /api/real-orders` + `GET /api/real-orders/{id}` (read-only, POST/PUT/DELETE 0건)
- [x] `app/api/schemas.py`: `RealOrderSchema` / `RealFillSchema` / `RealOrderDetailResponse` 추가
      (FORBIDDEN 응답 필드: `broker_order_no_hash` / `api_key` / `secret` / `account_number` / `raw_response` 0건)
- [x] `app/api/__init__.py` + `app/main.py`: `real_order_router` 등록

프런트엔드 (15 번째 화면 `/real-orders`):
- [x] `frontend/src/api/types.ts`: `RealOrder` / `RealFill` / `RealOrderDetailResponse` 타입 추가
      (FORBIDDEN 필드 0건 — broker_order_no_hash / api_key / raw_response / account_number / real_account)
- [x] `frontend/src/api/realOrders.ts` (신규): `fetchRealOrders` / `fetchRealOrderDetail` 2 fetch 함수
- [x] `frontend/src/hooks/useRealOrders.ts` (신규): `useRealOrders` / `useRealOrderDetail` 2 read-only hooks (mutation 0건)
- [x] `frontend/src/pages/RealOrders/index.tsx` (신규, 15 번째 화면):
      `RealOrdersPage` / `RealTradingSafetyBanner` ("dry-run / 기록 조회 전용" 항상 표시) /
      `RealOrderSummaryCards` (총 주문 / Dry-run / 체결 완료) / `RealOrdersTable` / `RealOrderDetailPanel` /
      `RealFillsTable`. "실주문 실행" / "자동매매" actionable CTA 0건
- [x] `frontend/src/router.tsx` + `frontend/src/components/layout/Sidebar.tsx`:
      15 번째 메뉴 `/real-orders` (`TrendingUp` 아이콘, "실주문 준비 (β)"). footer "v0.16 Real Order Integration Skeleton" 갱신
- [x] `frontend/src/tests/RealOrders.test.tsx` (신규) 13 건:
      page render / safety banner / empty state / orders table / DRY_RUN badge / summary cards /
      detail panel / fills table / fills empty / error state / forbidden field 미렌더링 / 자동매매 CTA 없음 / mutation hook 0건
- [x] `frontend/src/tests/mswServer.ts`: real-orders 2 mock 추가
- [x] `frontend/e2e/fixtures/apiMocks.ts`: real-orders 2 mock 추가
- [x] `frontend/e2e/dashboard.spec.ts`: sidebar 14→**15** + `/real-orders` navigation +
      safety banner + empty state + forbidden DOM 토큰 0건 단언

문서:
- [x] `RELEASE_NOTES_v0.16.md` (신규) — Phase A~E 산출물 + 최종 게이트 + 안전 정책 + v0.17 후보
- [x] `README.md` v0.16-final 배너 / 기능 목록 / 누적 사이클 표 / 회귀 기준선 갱신
- [x] `PROJECT_STATUS.md` §0 v0.16 마감 선언 (시작 선언 → 마감 선언으로 갱신)
- [x] `ROADMAP.md` v0.16 행 ✅ 마감
- [x] `TASKS.md` / `ARCHITECTURE.md` / `TESTING.md` / `API_SPEC.md` / `INTEGRATION_RUNBOOK.md` 갱신
- [x] 최종 4 게이트 확인 — pytest **1905** / vitest **214** / e2e **24** / build 그린
- [x] `tag v0.16-final + push`

---

## v1.0 — Small Approval Trading Release

세부 계획: [`PLANS.md`](./PLANS.md) `PLAN-0017`. 채택 시나리오: **Scenario X — Small Approval Trading
Release (소액 승인 기반 실주문 + KIS 실 transport + RealOrderExecutor real path + idempotent Fill
Sync)**. 기준선 `v0.16-final` (HEAD `2d262d5`, pytest 1905 / vitest 214 / e2e 24 / build 그린).
...
- [x] `app/broker/kis_order_transport_real.py` (신규) — `HttpxKisOrderTransport(KisOrderClientInterface)`
      실 구현 (3 메서드: `place_order` / `query_fill_status` / `cancel_order`)
- [x] lazy `httpx.Client` import — module-level httpx import 0건, `__init__` 내부에서만 import
      (AST 가드로 단언). v0.11 `HttpxDartTransport` 패턴 준수
- [x] timeout 정책 per-call: `kis_order_place_timeout_s=5.0` / `kis_order_query_timeout_s=10.0` /
      `kis_order_cancel_timeout_s=5.0` (env override `KIS_ORDER_PLACE_TIMEOUT_S` /
      `_QUERY_TIMEOUT_S` / `_CANCEL_TIMEOUT_S`). `__post_init__` 양수 검증
- [x] retry 정책 (pure helper `_run_with_retries`): place_order **재시도 0건** (`_PLACE_ORDER_MAX_RETRIES=0`,
      중복 주문 위험) / query_fill_status·cancel_order 최대 2회 (transient TimeoutException / NetworkError 만)
- [x] 민감 필드 마스킹: `mask_sensitive_order_payload()` (v0.16) + `install_sensitive_qs_filter("httpx")` (v0.11
      `SensitiveQueryStringFilter`). `__repr__` / `as_dict()` / 로그 caplog 검증 — app_key / app_secret /
      access_token / account_no 평문 0건. account_no 는 `****<last4>` 형태로만 노출
- [x] 응답 분류:
      - `place_order` 5종: SUBMITTED / REJECTED / TIMEOUT / NETWORK_ERROR / UNKNOWN
      - `query_fill_status` 6종 (FULL/PARTIAL/NONE/REJECTED/CANCELED/UNKNOWN) → 데이터클래스 5종
        (`FILLED`/`PARTIALLY_FILLED`/`PENDING`/`REJECTED`/`CANCELED`) 매핑
      - `cancel_order` 5종: CANCELED / REJECTED / TIMEOUT / NETWORK_ERROR / UNKNOWN
- [x] raw KIS response 0건 — whitelist 필드만 (`output.ODNO` / `rt_cd` / `msg1` / `output1.ord_qty` /
      `tot_ccld_qty` / `cncl_yn` / `rjct_yn`) immutable result 데이터클래스로 재투영. broker_order_no 는
      result `order_no` 필드로만 노출 (상위 계층이 SHA-256 해싱 후 저장)
- [x] `app/broker/__init__.py` — `HttpxKisOrderTransport` / `PlaceClassification` /
      `FillClassification` / `CancelClassification` export 추가
- [x] `app/config/settings.py` 신규 4 settings (`KIS_ORDER_BASE_URL` 기본 paper VTS endpoint /
      `KIS_ORDER_PLACE_TIMEOUT_S=5.0` / `KIS_ORDER_QUERY_TIMEOUT_S=10.0` / `KIS_ORDER_CANCEL_TIMEOUT_S=5.0`).
      신규 인증 settings 추가 0건 (기존 `KIS_APP_KEY` / `KIS_APP_SECRET` / `KIS_ACCOUNT_NO` 재사용)
- [x] `tests/unit/test_kis_order_transport_real.py` (신규) 53 건:
      §1 ABC 일치 / §2 AST 가드 4종 (no module-level httpx / no requests / no urllib / no providers.kis) /
      §3 repr·str·as_dict 자격증명 미노출 4종 / §4 place_order 8종 (success / business reject / 4xx / 5xx /
      timeout no-retry / network no-retry / invalid JSON / SELL/BUY tr_id 분기) / §5 query_fill_status 10종
      (FULL/PARTIAL/NONE/CANCELED/REJECTED/no-output/business-error/retry-success/retry-exhausted/4xx-no-retry/
      network-retries) / §6 cancel_order 7종 (success/business-reject/4xx/5xx/timeout/network-retry/
      retry-second-attempt) / §7 raw response 미노출 2종 (place + query) / §8 마스킹 (caplog) 2종
      (헤더가 서버 도달은 성공, 로그는 평문 0건) / §9 retry helper pure 단위 4종 / §10 respx-mock 통합 1종 /
      §11 settings 경계 4종 / §12 broker __init__ re-export / §13 Phase C/D scope guard 2종 / §14 close 2종
- [x] `tests/unit/test_safety_settings.py` Phase B scope guard 갱신:
      `test_v10_phase_a_httpx_kis_order_transport_module_absent` →
      `test_v10_phase_b_httpx_kis_order_transport_module_present` (assertion 방향 반전).
      `kis_order_client.py` 는 여전히 httpx-free / `HttpxKisOrderTransport` 정의 0건 단언 유지.
      Phase C (executor real path) / Phase D (fill_sync real) scope guard 4 종 유지
- [x] AST 가드: 모듈 최상단 `import httpx` 0건 (lazy `__init__` 내부만) / `requests` 0건 /
      `urllib` 0건 / `app.providers.kis` 0건 / `app.data.collectors.kis_client` 0건
- [x] respx + httpx.MockTransport 100% — 실 KIS endpoint 호출 0건. 모든 53 테스트가 mock transport 또는
      respx route 사용
- [x] pytest **1942 → 1995 passed (+53)** / 회귀 0건 / Alembic revision 0건 / DB 모델 0건 / API 라우터 0건 /
      프런트 0건 / 신규 pip 0건 (`httpx` v0.11 부터 production / `respx` v0.11 부터 dev 의존성 그대로)
- [ ] `tag v1.0-kis-real-transport + push`

### Phase C — RealOrderExecutor real path behind strict gates ✅

- [x] `app/broker/real_order_executor.py` 갱신 — v0.16 8-gate → v1.0 **10-gate + 분기**:
      gate 4 신규 (TRADING_SAFETY_DISABLED — v0.15 layer 명시 검증) +
      gate 10 신규 (TRANSPORT_UNAVAILABLE — real path 진입 시 non-Fake transport 보장,
      factory 사용 가능성 + `_resolve_real_transport()` private helper)
- [x] gate ordering: ① CANDIDATE_NOT_FOUND/NOT_APPROVED → ② DUPLICATE_REAL_ORDER/ALREADY_EXECUTED →
      ③ KILL_SWITCH_ON → ④ TRADING_SAFETY_DISABLED → ⑤ REAL_TRADING_DISABLED → ⑥ KIS_ORDER_DISABLED →
      ⑦ AMOUNT_EXCEEDS_PER_ORDER_CAP → ⑧ AMOUNT_EXCEEDS_DAILY_CAP → ⑨ RISK_REJECTED →
      ⑩ TRANSPORT_UNAVAILABLE (real path only). dry-run 은 ①~⑨ 만, real path 는 ①~⑩ 모두
- [x] `real_order_dry_run` 분기:
      - True → 기존 dry-run 경로 (FakeTransport → `dry_run=True`, status=DRY_RUN) 그대로
      - False → **real path**: `RealOrder(dry_run=False, status=CREATED)` 선저장 → `session.flush()`
        (DB anchor 보장) → `transport.place_order(KisOrderRequest)` 1 회 호출
        (Phase B retry=0 정책 보존) → 분류:
        SUBMITTED → `mark_submitted(broker_order_no_hash=sha256(order_no))` /
        REJECTED → `mark_failed(error_code="REJECTED", error_message=scrub(msg))` /
        TIMEOUT/NETWORK_ERROR/UNKNOWN → `mark_failed(error_code=cls)`
- [x] `app/data/repositories/real_order.py` — `exists_non_failed_for_candidate(candidate_id)` 헬퍼 추가
      (`_NON_FAILED_STATUSES = {DRY_RUN, CREATED, SUBMITTED, PARTIALLY_FILLED, FILLED}`).
      raw 저장 메서드 추가 0 건
- [x] `app/data/repositories/approval_audit_log.py` — `VALID_EVENT_TYPES` 에
      `REAL_ORDER_SUBMITTED` / `REAL_ORDER_FAILED` 2 종 추가. forbidden details keys 정책 그대로 유지 —
      `real_order_id` / `kis_order_id` / `api_key` 등 기존 13 종 차단 정책 변경 0 건
- [x] real path audit 기록 — SUBMITTED/FAILED 분기 직후 `ApprovalAuditLog.append()` 1 행
      (whitelist details: `classification` / `dry_run` / `symbol` / `side` / `quantity` /
      `broker_order_no_hash_prefix` (16자, plaintext 0건)). 실패 시에도 audit 1 행 보장
- [x] `_scrub()` helper — KIS msg 가 평문 `api_key` / `appsecret` / `access_token` /
      `authorization` / `account_no` / `secretkey` 6 substring 포함 시 case-insensitive `***` 치환 후
      `mark_failed(error_message=...)` 호출. RealOrderRepository 의 sensitive substring 검증 통과 보장
- [x] `RealOrderExecutor.__init__` 확장: `transport=` (기본 FakeKisOrderTransport) +
      `real_transport_factory: Callable[[Settings], KisOrderClientInterface] | None` (선택). 기본 생성자는
      여전히 Fake-only — v0.16 회귀 0 건
- [x] dry-run fallback — `real_order_dry_run=True` 또는 `KIS_ORDER_ENABLED=false` 시 자동으로
      dry-run 경로 (FakeTransport) — `_resolve_real_transport()` 가 None 반환 시 gate 10 차단으로
      ExecutorResult(blocked=True, reason="TRANSPORT_UNAVAILABLE")
- [x] `ExecutorResult` schema — `success: bool` / `dry_run: bool` / `blocked_reason: str | None` /
      `real_order_id: int | None` / `status: str` ("DRY_RUN" / "BLOCKED" / "SUBMITTED" / "FAILED") /
      `message: str`. v0.16 dataclass 그대로 유지, status 값에 SUBMITTED/FAILED 2 종 추가
- [x] plaintext order_no 저장·반환·로그 0 건 — `RealOrder` 어떤 컬럼에도 plaintext 미존재 (단언),
      `ExecutorResult.message` 도 hash-only / status-only
- [x] `tests/unit/test_real_order_executor.py` v1.0 Phase C 단위 테스트 +37 건 추가:
      §16 gate 4 TRADING_SAFETY_DISABLED + gate ordering / §17 gate 10 TRANSPORT_UNAVAILABLE 4 종
      (no transport / factory→Fake / factory raises / dry-run skips gate 10) / §18 real path SUBMITTED
      3 종 (creates RealOrder / no plain order_no / message no leak) / §19 real path 비-SUBMITTED
      4 분기 parametrize (REJECTED/TIMEOUT/NETWORK_ERROR/UNKNOWN) / §20 audit 4 종 (event_types 확장 /
      SUBMITTED writes audit / FAILED writes audit / no forbidden keys) / §21 repo helper 7 종
      (false-empty / true-non-failed parametrize 5 / false-failed parametrize 3 / duplicate blocks /
      failed allows retry) / §22 sensitive substring scrubbing / §23 anchor row before transport call /
      §24 AST 가드 2 종 (no kis_order_transport_real / no app.providers.kis) / §25 constructor 2 종
      (factory takes precedence / default Fake-only) / §26 Phase D scope guard
- [x] `tests/integration/test_real_order_executor_integration.py` (신규) 8 건 — `respx` mock + DB 통합:
      SUBMITTED end-to-end / 4xx REJECTED / 5xx UNKNOWN / TIMEOUT / business-error rt_cd≠0 /
      anchor row before KIS call / duplicate blocks second attempt / dry-run + real coexistence
- [x] `tests/unit/test_safety_settings.py` Phase C scope guard 전환 —
      `test_v10_phase_c_no_real_order_executor_real_path` →
      `test_v10_phase_c_real_order_executor_real_path_present` (assertion 반전, `_execute_real` /
      `_resolve_real_transport` private 메서드 존재 단언)
- [x] AST 가드: `real_order_executor.py` 직접 `httpx.Client` 0 건 / `httpx` import 0 건 /
      `requests` / `urllib` / `app.providers.kis` / `app.broker.kis_order_transport_real` 직접 import 0 건
      (transport 는 DI 만)
- [x] pytest **1995 → 2040 passed (+45)** / 회귀 0 건 / Alembic head `0010_real_fills` 변경 0 건 /
      DB 모델 0 건 / API 라우터 0 건 / 프런트 0 건 / 신규 pip 0 건 / 실 KIS API 호출 0 건 /
      외부 네트워크 호출 0 건
- [ ] `tag v1.0-real-order-executor-real + push`

### Phase D — Fill Sync actual transport + idempotent RealFill update ✅

- [x] `app/broker/fill_sync_service.py` 갱신 — v0.16 mock-only → v1.0 **델타 기반 idempotent + 6 분류 + audit**:
      `existing_total = RealFillRepository.total_filled_quantity(order_id)` /
      `kis_total = _effective_kis_total(classification, response, total_qty)` /
      `delta = kis_total - existing_total`
      - delta > 0 → `RealFill(quantity=delta, ...)` 1 행 생성 + ApprovalAuditLog `REAL_ORDER_FILL_SYNCED`
      - delta == 0 → 신규 행 0건 (idempotent) + audit 1 행
      - delta < 0 → audit `FILL_SYNC_NEGATIVE_DELTA` + return FAILED
- [x] 6 분류 (`_classify_fill_response`): FULL / PARTIAL / NONE / REJECTED / CANCELED / FAILED.
      `RealOrder.status` 자동 전이 — FULL→FILLED / PARTIAL→PARTIALLY_FILLED / REJECTED→REJECTED /
      CANCELED→CANCELED. NONE/FAILED → 상태 변경 없음
- [x] DRY_RUN RealOrder 는 transport 호출 0건으로 **skip** — `fill_status="NONE"` /
      `skipped_reason="DRY_RUN_ORDER_SKIPPED"`. 실제 KIS 의 fake order_no 충돌 위험 차단 (v1.0 정책)
- [x] `app/data/repositories/approval_audit_log.py` `VALID_EVENT_TYPES` 에 3 종 추가:
      `REAL_ORDER_FILL_SYNCED` / `REAL_ORDER_FILL_FAILED` / `FILL_SYNC_NEGATIVE_DELTA`. forbidden
      details keys 13 종 정책 변경 0건
- [x] `app/data/repositories/real_fill.py` — `total_filled_quantity(real_order_id) -> int` 헬퍼 추가
      (DB-level `func.sum()` aggregate, 상수 시간)
- [x] `app/api/real_order_routes.py` 신규 mutation — `POST /api/real-orders/{order_id}/sync`:
      AUTH (`require_auth`) + `_require_trading_safety_enabled()` + `_require_kill_switch_off()` 3중 게이트
      (v0.15 `approval_routes` 패턴 동일). `REAL_TRADING_ENABLED` / `KIS_ORDER_ENABLED` 는 의도적으로
      **미강제** — 운영자가 사고 후 sync 가능해야 함 (RUNBOOK §6 기준)
- [x] `app/api/schemas.py` — `RealOrderSyncRequest` (옵션 plaintext kis_order_no) +
      `RealOrderSyncResponse` (`real_order_id` / `real_order_status` / `fill_status` / `fills_added` /
      `fills_total` / `synced_at` / `message`) 신규
- [x] plaintext kis_order_no 정책: 응답 본문에서 echo 0건. transport 까지 in-memory 만 통과,
      `RealFill` / `RealOrder` / audit 어디에도 저장 0건. 단언 통합 테스트 2 건
- [x] `tests/unit/test_fill_sync_service.py` v1.0 케이스 +24 건 (§11~§18):
      audit event_type 확장 1 / delta 5 종 (FULL first / repeat idempotent / partial→full /
      partial idempotent / pending) / negative delta 1 / REJECTED·CANCELED 전이 2 / FAILED 2 /
      audit details no-forbidden 1 / plaintext never persisted 1 / Phase D scope guard 2.
      기존 v0.16 테스트는 v1.0 semantics 에 맞춰 갱신 (DRY_RUN 시드 → SUBMITTED 시드 + DRY_RUN skip 단언)
- [x] `tests/integration/test_fill_sync_integration.py` (신규) 7 건 — `httpx.MockTransport` + DB end-to-end:
      FULL / PARTIAL→FULL / 반복 idempotent / negative delta / 5xx FAILED / plaintext never persisted /
      CANCELED 전이
- [x] `tests/integration/test_real_order_sync_api.py` (신규) 10 건 — `POST /sync` HTTP:
      200 happy / 200 idempotent (already FILLED) / 200 DRY_RUN skip / 404 / 503 trading_safety /
      503 kill_switch / 401 AUTH / 405 PUT/PATCH/DELETE parametrize 3 / plaintext body 미 echo /
      forbidden substring scan / route 등록 1
- [x] `tests/integration/test_auth_security.py` — mutating endpoint count 15 → **16**
      (real-orders 1 추가). `_ALLOWED_ORDER_PREFIXES` 에 `/api/real-orders` 이미 포함 — 변경 0건
- [x] `tests/unit/test_safety_settings.py` Phase D scope guard 전환 (assertion 반전):
      `_no_fill_sync_real_transport_helper` → `_fill_sync_delta_idempotency_present` (`total_filled_quantity`
      헬퍼 존재 단언) + `_audit_event_types_extended` + `_sync_route_registered` +
      `_no_auto_polling_scheduler_job` + `_no_reconciliation_module_yet`
- [x] AST 가드: `fill_sync_service.py` 직접 `httpx.Client` 0건 / `httpx` import 0건 / `requests` /
      `urllib` / `KisHttpOrderTransport` 직접 reference 0건. transport 는 항상 DI
- [x] Alembic head: `0010_real_fills` 변경 0건 (41 테이블 그대로). DB 모델 변경 0건. compare_metadata diff 0건
- [x] pytest **2040 → 2082 passed (+42)** / 회귀 0건 / Alembic 0건 / DB 모델 0건 / 프런트 0건 /
      신규 pip 0건 (`httpx` / `respx` / stdlib 만) / 실 KIS API 호출 0건 / 외부 네트워크 호출 0건 /
      raw KIS response 저장·반환 0건 / API key / app_secret / access_token / account_no /
      plaintext order_no 평문 저장·로그·응답 0건 / 자동 polling job 0건 / Reconciliation 0건
- [ ] `tag v1.0-fill-sync-real + push`

### Phase E — Runbook / UI status polish / v1.0-final release ✅

backend (최소 변경):
- [x] `app/api/schemas.py` `SettingsResponse` — 5 신규 bool 필드 (`trading_safety_enabled` /
      `kill_switch_enabled` / `real_trading_enabled` / `kis_order_enabled` / `real_order_dry_run`).
      secret / api_key / account_no 평문 노출 0건 유지
- [x] `app/api/routes.py` `/api/settings` builder — 위 5 bool wiring 추가

프런트엔드 (15 번째 화면 강화):
- [x] `frontend/src/api/types.ts` — `SettingsResponse` 5 신규 bool +
      `RealOrderSyncRequest` / `RealOrderSyncResponse` / `RealOrderFillStatus` 신규 타입
- [x] `frontend/src/api/realOrders.ts` — `syncRealOrder(orderId, body?)` 함수 추가 (apiPost 래퍼)
- [x] `frontend/src/hooks/useRealOrders.ts` — `useSyncRealOrder` mutation hook
      (real-orders namespace invalidate, retry: false, RUNBOOK §6 manual followup)
- [x] `frontend/src/pages/RealOrders/index.tsx` —
      ...
      isPending spinner, success/error 메시지, 라벨 "체결 동기화" / "Sync Fill" 만)
- [x] `frontend/src/components/layout/Sidebar.tsx` — footer "v0.16 Real Order Integration Skeleton" →
      "v1.0 Small Approval Trading Release", "v0.16 dashboard" → "v1.0 dashboard"
- [x] `frontend/src/tests/RealOrders.test.tsx` Phase E 테스트 +11 건 (banner 5 flag badge / mode 4 종 /
      Sync 버튼 가시성 + disabled 4 케이스 / mutation happy / mutation 503 error / forbidden CTA / plaintext)
- [x] `frontend/src/tests/mswServer.ts` — `/api/settings` 5 bool + `POST /sync` 503 mock 추가
- [x] `frontend/e2e/fixtures/apiMocks.ts` — `SETTINGS_SAFE` 에 5 bool + `POST /sync` 503 핸들러
- [x] `frontend/e2e/dashboard.spec.ts` — Real Orders e2e 강화 (5 flag badge + mode badge 단언) +
      Sync Fill 버튼 e2e 1 건 신규 (paranoid 기본 → disabled, KILL_SWITCH_ENABLED reason 단언, forbidden CTA 0건)
- [x] forbidden DOM 토큰 단언 확장: `broker_order_no` / `broker_order_id` / `kis_order_id` 추가 — 14 종 0 건 단언
- [x] forbidden CTA 단언 그대로: "place real order" / "자동매매" / FULL_AUTO / SMALL_AUTO 0건. "실주문 실행" 버튼 없음

문서:
- [x] `RELEASE_NOTES_v1.0.md` 본 마감 — Phase A~E 산출물 + 4 게이트 변동 표 + v1.0 안전 정책 +
      실 KIS 호출 테스트 정책 + 알려진 한계 + v1.1 후보 9 항목 + 누적 태그 정리
- [x] `RUNBOOK_REAL_TRADING.md` v1.0-final 마커 + §1~9 잠금 (§8 운영 체크리스트 5 카테고리 25 항목)
- [x] `README.md` v1.0-final 배너 / 기능 목록 / 누적 사이클 표 / 회귀 기준선 갱신
- [x] `PROJECT_STATUS.md` § 0 v1.0 시작 선언 → 마감 선언 갱신 (이전 § 0 → § 0-1 강등 별개 — Phase A~E 누적 §0 갱신만)
- [x] `ROADMAP.md` v1.0 행 ✅ 마감 + Phase A~E 결과 표 갱신 + 장기비전 다이어그램 v1.0 ✅ 갱신
- [x] `TASKS.md` Phase E 체크박스 전체 완료
- [x] `ARCHITECTURE.md` / `TESTING.md` / `API_SPEC.md` / `INTEGRATION_RUNBOOK.md` 갱신
- [x] 최종 4 게이트 확인 — backend pytest **2082** / frontend vitest **225** / e2e **25** / build 그린
- [ ] `tag v1.0-final + push`
...
자세한 분류는 [`ROADMAP.md`](./ROADMAP.md) / [`PROJECT_STATUS.md`](./PROJECT_STATUS.md) 참조.

- **Reconciliation (v1.1 핵심 후보)** — 내부 `RealOrder` / `RealFill` vs KIS 잔고/체결 비교 + mismatch 감지 +
  mismatch 발생 시 KillSwitch 자동 활성화 + `reconciliation_runs` / `reconciliation_mismatches` 신규 테이블
  (Alembic 1~2 건). 진입 전제: v1.0 운영 4 주 + 실주문 ≥10 건
- **PreTradeRiskEngine 신규 룰 (v1.1 후보)** — 호가 jump 검사 / 시장 시간 외 차단 / 종목당 일일 한도 /
  연속 실패 종목 차단
- **자동 Fill Sync 폴링 잡 (v1.1 후보)** — `SUBMITTED` 상태 RealOrder 가 N 분 이상 갱신 없으면 자동 sync.
  `KIS_ORDER_ENABLED=true` 시에만
- **`submitted_at` / `filled_at` timestamps + idempotency key (v1.1 후보)** — Alembic 1 건 (`0011_real_order_v1_fields`)
- **`RealOrder` 운영 화면 actionable CTA (v1.1 후보)** — "Cancel" 버튼. v1.0 에서는 "Sync Fill" 만. 운영자 CLI 보강
...
- **Grafana dashboard JSON 동봉** — v0.11 Prometheus exporter 위 시각화 layer
- **ProviderHealthMonitor 영속화** — DB / Redis 백업으로 재시작 후 history 유지
- **인증 고도화** — refresh token / 다중 사용자 / OAuth / RBAC
- **LLM sentiment / 자동 요약** — 룰 기반 검증 후
- **WebSocket / SSE 실시간 갱신** — Provider Health / 백테스트 진행
- **모바일 / 태블릿 / lightweight-charts 마이그레이션** — UX 고도화
- **Future Backlog (자동매매)** — FULL_AUTO / SMALL_AUTO 모두 별도 보안·컴플라이언스 사이클 선행 필수.
  본 프로젝트 명시 금지 정책 그대로
