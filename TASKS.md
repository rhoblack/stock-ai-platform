...
## v0.7 — Backlog (마감 후 검토 대기)

자세한 분류는 [`ROADMAP.md`](./ROADMAP.md) §7 참조. 한 줄 요약:

- **데이터 / 분석 실제화** — 실 DART API (`DartFundamentalProvider` / `DartEarningsProvider`), 실 RSS / News API, 재무 score percentile 정규화, earnings surprise 다지표 가중, LLM 보강
- **인증 / 관심종목** — 단일 토큰 → POST 라우터 → Watchlist (인증 동반 필수)
- **운영 / UX** — Sentry / Prometheus / Grafana, 모바일 레이아웃, lightweight-charts 마이그레이션, Alembic 도입
- **백엔드 인프라** — POST 트리거, WebSocket / SSE 실시간 잡 상태, dependabot
