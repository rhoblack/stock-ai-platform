...
프런트엔드 (15 번째 화면 강화):
- [ ] `frontend/src/pages/RealOrders/index.tsx` — `RealTradingModeBanner` 추가 (DRY-RUN / REAL TRADING /
      전이 상태 3종 배지) + `RealOrderDetail` 에 "Sync Fill" 버튼 (KillSwitch ON 시 disabled, dry-run
      RealOrder 에서 disabled)
- [ ] `frontend/src/api/realOrders.ts` — `syncRealOrder` 함수 추가
- [ ] `frontend/src/hooks/useRealOrders.ts` — `useSyncRealOrder` mutation hook (real-orders namespace invalidate)
- [ ] `frontend/src/components/layout/Sidebar.tsx` — footer "v1.0 Small Approval Trading Release" 갱신
- [ ] `frontend/src/tests/RealOrders.test.tsx` — Sync 버튼 / banner / mutation hook +5 건
- [ ] `frontend/src/tests/mswServer.ts` — sync mock 추가
- [ ] `frontend/e2e/dashboard.spec.ts` — `/real-orders` 에서 SafetyBanner / sync 버튼 (KillSwitch ON 일 때 disabled)
      단언. e2e 24 → 25 (+1)
- [ ] forbidden DOM 토큰 단언 그대로: "place real order" / "자동매매" / FULL_AUTO / SMALL_AUTO 0건. "실주문 실행" 버튼 없음

문서:
- [ ] `RELEASE_NOTES_v1.0.md` (신규) — Phase A~E 산출물 + 최종 게이트 + v1.0 안전 정책 + 운영 진입 체크리스트 + v1.1 후보
- [ ] `RUNBOOK_REAL_TRADING.md` (Phase A 신규) 최종 확정 — 운영 진입 체크리스트 잠금
- [ ] `README.md` v1.0-final 배너 / 기능 목록 / 누적 사이클 표 / 회귀 기준선 갱신
- [ ] `PROJECT_STATUS.md` § 0 v1.0 마감 선언 (시작 선언 → 마감 선언으로 갱신, 이전 § 0 → § 0-1 강등)
- [ ] `ROADMAP.md` v1.0 행 ✅ 마감 + v1.1+ Backlog 정리
- [ ] `TASKS.md` Phase E 체크박스 전체 완료
- [ ] `ARCHITECTURE.md` / `TESTING.md` / `API_SPEC.md` / `INTEGRATION_RUNBOOK.md` 갱신
- [ ] 최종 4 게이트 확인 — pytest **~2025** / vitest **219** / e2e **25** / build 그린
- [ ] `tag v1.0-final + push`
