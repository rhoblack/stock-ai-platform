...
- [ ] `app/broker/kis_order_transport_real.py` (신규 권장) — `HttpxKisOrderTransport(KisOrderClientInterface)`
      실 구현 (3 메서드: `place_order` / `query_fill_status` / `cancel_order`)
- [ ] lazy `httpx.Client` import + factory 자동 주입 패턴 (v0.11 `HttpxDartTransport` 와 동일)
- [ ] timeout 정책: connect 5s / read 10s / write 5s — `KIS_ORDER_TIMEOUT_*` env override 가능
- [ ] retry 정책: place_order 재시도 0건 (중복 주문 위험) / query_fill_status·cancel_order 최대 2회
- [ ] 민감 필드 마스킹: `mask_sensitive_order_payload()` (v0.16 helper) + `SensitiveQueryStringFilter` (v0.11 패턴) —
      app_key / app_secret / access_token / account_number / cano / acnt_prdt_cd 모두 평문 0건
- [ ] 응답 → `KisOrderResult` 매핑 5 분류 (SUBMITTED / REJECTED / TIMEOUT / NETWORK_ERROR / UNKNOWN)
- [ ] broker_order_no SHA-256 (`broker_order_no_hash`) — 평문 저장 0건
- [ ] `app/broker/__init__.py` — `HttpxKisOrderTransport` export 추가
- [ ] `app/config/settings.py` 신규 4 settings (`KIS_ORDER_BASE_URL` / `KIS_ORDER_TIMEOUT_CONNECT=5` /
      `KIS_ORDER_TIMEOUT_READ=10` / `KIS_ORDER_TIMEOUT_WRITE=5`)
- [ ] `tests/unit/test_kis_order_transport_real.py` (신규) `respx` mock 기반 ~30 건 — happy path / timeout /
      4xx (REJECTED) / 5xx (UNKNOWN) / 응답 누락 / 마스킹 / 5 분기 / AST 가드
- [ ] `tests/unit/test_kis_order_client.py` Phase C scope guard 갱신 (real_order_executor real path 미존재 단언)
- [ ] AST 가드: `HttpxKisOrderTransport` 외에 직접 `httpx.Client(...)` 호출 0건 / `from app.providers.kis` 0건
- [ ] monkeypatch 가드: 실 KIS endpoint 호출 0건 (`respx` 가 모든 route 검증)
- [ ] pytest `~1915 → ~1945` (+30) / 회귀 0건 / Alembic 0건 / 신규 pip 0건 (`httpx` / `respx` 모두 기존)
