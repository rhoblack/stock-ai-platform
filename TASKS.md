...
- [ ] `app/broker/real_order_executor.py` 갱신 — v0.16 8-gate → v1.0 **10-gate + 분기**:
      gate 9 (중복 가드: 동일 `order_candidate_id` 비-FAILED RealOrder 존재 시 차단) +
      gate 10 (transport 가용성: `HttpxKisOrderTransport` factory init 실패 시 차단)
- [ ] gate 7 (`real_order_dry_run`) 분기:
      - True → 기존 dry-run 경로 (FakeTransport → `is_dry_run=True`) 그대로
      - False → real path: `RealOrder(is_dry_run=False, status=PENDING)` 선저장 →
        `HttpxKisOrderTransport.place_order()` → 응답에 따라 `update_status(SUBMITTED|REJECTED|FAILED)` +
        `broker_order_no_hash` set
- [ ] 실패 시 `ApprovalAuditLog.append(event_type='REAL_ORDER_FAILED', candidate_id=..., details={...})` 1 행
- [ ] dry-run fallback — `KIS_ORDER_ENABLED=false` 시 FakeTransport 자동 분기 (현재 동작 유지)
- [ ] `ExecutorResult` schema 그대로 (`blocked: bool` / `reason: str | None` / `real_order: RealOrder | None` /
      `is_dry_run: bool`)
- [ ] `tests/unit/test_real_order_executor.py` v1.0 케이스 ~25 건 추가:
      real path SUBMITTED / REJECTED / FAILED (timeout) / DUPLICATE_REAL_ORDER / TRANSPORT_UNAVAILABLE /
      dry-run fallback / audit 1 행 / 회귀 8-gate
- [ ] `tests/integration/test_real_order_executor_integration.py` (신규/갱신) `respx` mock + DB 통합 ~10 건:
      호출 전 저장 + 호출 후 update + commit 순서 정확 / DB 일관성
- [ ] AST 가드: `RealOrderExecutor` 파일 내 직접 `httpx.Client` 0건 / `from app.providers.kis` 0건
- [ ] pytest `~1945 → ~1980` (+35) / 회귀 0건 / Alembic 0건 / API 라우터 0건 / 프런트 0건
