...
## 35. virtual_positions (v0.14 Phase C)

페이퍼 / 시뮬레이션 트레이딩의 (account, symbol) 보유 상태. PnLTracker /
SimulationBroker.execute_pending_orders 가 채운다. **broker_order_id /
kis_order_id / real_account / api_key / token / secret 컬럼 0건** (회귀 단언:
`tests/integration/test_virtual_pnl_engine.py::test_phase_c_models_have_no_forbidden_columns`).
...
| account_id | FK → virtual_accounts.id `ON DELETE CASCADE`, NOT NULL, index |
| symbol | String(32) NOT NULL index |
| quantity | Integer NOT NULL default 0 — 누적 순(net) long position. 음수 금지(repository validator: 숏셀링 미지원) |
| avg_cost | Numeric(18,4) NOT NULL default 0 — BUY 체결 시 cost basis blend 로 재계산 (수수료/슬리피지 포함) |
| realized_pnl | Numeric(18,4) NOT NULL default 0 — 누적 실현 손익. SELL 만 갱신 |
| created_at / updated_at | TimestampMixin |

**Unique**: `(account_id, symbol)` (`uq_virtual_positions_account_symbol`).
**Index**: `account_id`, `symbol`.

**Lifecycle**: 보유 수량이 0 이 되면 row 는 보존되지만 `avg_cost` 는 0 으로
리셋 (재진입 시 cost basis 가 깨끗하게 시작). `realized_pnl` 누적 이력 유지.

## 36. virtual_fills (v0.14 Phase C)

체결 1건당 1 row 의 immutable 기록. SimulationBroker.execute_pending_orders 가
체결 시 작성. 비용 4종(`fee` / `stamp_tax` / `slippage` / `gross_amount`) 을
별도 컬럼으로 보관해 리포트가 그대로 재현 가능하다.

| 컬럼 | 설명 |
|---|---|
| id | PK autoincrement |
| order_id | FK → virtual_orders.id `ON DELETE CASCADE`, NOT NULL, index |
| account_id | FK → virtual_accounts.id `ON DELETE CASCADE`, NOT NULL, index |
| symbol | String(32) NOT NULL index |
| side | String(8) NOT NULL — BUY / SELL |
| quantity | Integer NOT NULL > 0 |
| fill_price | Numeric(18,4) NOT NULL — 체결가 (Phase C 는 daily_prices.close 기준) |
| fee | Numeric(18,4) NOT NULL default 0 — 거래 수수료 (default 0.015%) |
| stamp_tax | Numeric(18,4) NOT NULL default 0 — 증권거래세 (매도 only, default 0.18%) |
| slippage | Numeric(18,4) NOT NULL default 0 — 슬리피지 추정 (default 0.05%) |
| gross_amount | Numeric(18,4) NOT NULL — fill_price × quantity (양수) |
| net_amount | Numeric(18,4) NOT NULL — 절대값 양수. BUY 는 cash_outflow(gross+fee+slippage), SELL 은 cash_inflow(gross-fee-stamp_tax-slippage). 방향은 `side` 가 인코딩 |
| filled_at | DateTime(timezone=True) NOT NULL index, default `utc_now` |

**Index**: `order_id`, `account_id`, `symbol`, `filled_at`,
`(account_id, symbol)` (`ix_virtual_fills_account_symbol`).

**Cascade**: VirtualOrder / VirtualAccount 삭제 시 자동 drop.

## 37. virtual_pnl_snapshots (v0.14 Phase C)

일별 계좌 PnL 요약. PnLTracker.create_daily_pnl_snapshot 가 작성/갱신.
Idempotent — 같은 (account_id, snapshot_date) 재호출 시 in-place upsert.
...
| cash_balance | Numeric(18,4) NOT NULL — 스냅샷 시점 현금 잔고 |
| market_value | Numeric(18,4) NOT NULL — 보유 포지션 시가총액 (open positions × close_price). 가격 없는 종목은 0 기여 (graceful) |
| total_value | Numeric(18,4) NOT NULL — `cash_balance + market_value` |
| realized_pnl | Numeric(18,4) NOT NULL — 모든 포지션 누적 실현 PnL |
| unrealized_pnl | Numeric(18,4) NOT NULL — `Σ (close × qty − avg_cost × qty)`. 가격 없는 종목은 0 기여 |
| created_at / updated_at | TimestampMixin |

**Unique**: `(account_id, snapshot_date)`
(`uq_virtual_pnl_snapshots_account_date`).
**Index**: `account_id`, `snapshot_date`,
`(account_id, snapshot_date)` (`ix_virtual_pnl_snapshots_account_date`).

> **운영 환경 마이그레이션 (v0.14 Phase C)**: Alembic revision
> `0006_virtual_positions` 가 세 테이블 + 인덱스를 생성한다.
> `0005_virtual_trading_core` 위에 layering — `alembic upgrade head` 한 번
> 이면 적용. `compare_metadata` diff 0건 (CI 강제). 다운그레이드
> (`alembic downgrade 0005_virtual_trading_core`) 는 `virtual_pnl_snapshots`
