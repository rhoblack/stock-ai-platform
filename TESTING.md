...
> 본 문서는 **v0.15 Phase A 시점** 기준으로 갱신되었다 (`v0.15-safety-settings`
> 태그 예정). 누적 cycle 의 게이트 baseline 과 v0.4–v0.15 신규 테스트 카테고리를
> 반영한다.

## 1. 현재 회귀 게이트 (v0.15 Phase A 완료 시점)
...
| 게이트 | 명령 | 현재 baseline |
|---|---|---|
| backend pytest | `python -m pytest -q` | **1498 passed** (v0.15 Phase A: 1438 → 1498, +60 — `test_safety_settings.py` 신규 + `test_project_structure.py` 7 default 단언 추가) |
| frontend vitest | `cd frontend && npm run test -- --run` | **186 passed** — 변경 없음 (Phase A 는 backend settings only) |
| frontend build | `cd frontend && npm run build` | 그린 (`tsc --noEmit && vite build`) |
| Playwright e2e | `cd frontend && npm run e2e` | **22 passed** — 변경 없음 (Phase A 는 backend settings only) |
