...
> 본 문서는 **v0.14 Phase B 시점** 기준으로 갱신되었다 (`v0.14-sim-broker`
> 태그 예정). 누적 cycle 의 게이트 baseline 과 v0.4–v0.14 신규 테스트 카테고리를
> 반영한다.

## 1. 현재 회귀 게이트 (v0.14 Phase B 완료 시점)
...
| 게이트 | 명령 | 현재 baseline |
|---|---|---|
| backend pytest | `python -m pytest -q` | **1365 passed** (v0.14 Phase B: 1322 → 1365, +43 — SimulationBroker 단위 29건 + virtual trading core 통합 16건 — 1건 alembic spot-check 1건은 기존 테스트 파라미터화로 흡수) |
| frontend vitest | `cd frontend && npm run test -- --run` | **175 passed** (v0.13 Phase D: 165 → 175, +10 — v0.14 Phase B 미변경) |
| frontend build | `cd frontend && npm run build` | 그린 (`tsc --noEmit && vite build`) |
| Playwright e2e | `cd frontend && npm run e2e` | **21 passed** (v0.14 Phase B 미변경) |
