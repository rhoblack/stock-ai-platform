...
> 본 문서는 **v0.14 Phase D 시점** 기준으로 갱신되었다 (`v0.14-paper-api`
> 태그 예정). 누적 cycle 의 게이트 baseline 과 v0.4–v0.14 신규 테스트 카테고리를
> 반영한다.

## 1. 현재 회귀 게이트 (v0.14 Phase D 완료 시점)
...
| 게이트 | 명령 | 현재 baseline |
|---|---|---|
| backend pytest | `python -m pytest -q` | **1438 passed** (v0.14 Phase D: 1405 → 1438, +33 — paper API 25건 + paper scheduler 8건; auth_security mutating count + no_auto_trade_strings + scheduler registry sanity 갱신 흡수) |
| frontend vitest | `cd frontend && npm run test -- --run` | **175 passed** (v0.13 Phase D: 165 → 175, +10 — v0.14 Phase D 미변경) |
| frontend build | `cd frontend && npm run build` | 그린 (`tsc --noEmit && vite build`) |
| Playwright e2e | `cd frontend && npm run e2e` | **21 passed** (v0.14 Phase D 미변경) |
