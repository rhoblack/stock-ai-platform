...
provider transport injection + httpx.MockTransport + `_StubTransport` + `_ScriptedTransport` 6중 가드).
...
## 6.46 v0.15 Phase D — Approval Workflow API + Audit Log + Service 테스트

### 6.46.1 ApprovalAuditLog repository — `tests/integration/test_approval_audit_log.py` (17건)

- happy append + 256자 reason auto-truncate
- `event_type` 8종 parametrize (CREATED / RISK_CHECKED / RISK_REJECTED /
  APPROVED / REJECTED / EXPIRED / EXECUTED_PAPER / KILL_SWITCH_BLOCKED)
- 미지원 `event_type` 거부
- forbidden details key 14종 parametrize (`api_key / token / secret /
  access_token / jwt_secret / broker_order_id / kis_order_id / real_account /
  real_order_id / account_number / raw_text / body / full_text /
  source_file_path`)
- non-dict details 거부 / hash 65자 거부 (parametrize 2건)
- list_by_candidate ascending id / list_recent descending id / event filter /
  unknown event 거부
- **append-only 회귀**: repository public 메서드명에 `update / delete /
  remove / set_event / edit / patch / mutate` 키워드 0건
- `ApprovalAuditLog.__table__.columns` forbidden 컬럼 0건
- VirtualAccount → OrderCandidate → ApprovalAuditLog cascade delete

### 6.46.2 Approval API — `tests/integration/test_approval_api.py` (29건)

- GET candidates default → PENDING_APPROVAL 만 / detail + risk_check_result
  whitelist / 404 unknown / 422 status filter
- POST candidate **3중 게이트**: trading_safety=False → 503 (workflow disabled) /
  kill_switch=True → 503 / auth_enabled + Bearer 미제공 → 401
- POST candidate risk pass → status=PENDING_APPROVAL + risk_passed=True
- POST candidate risk fail (estimated > max_order_amount) → status=RISK_REJECTED +
  per_symbol_limit violation
- POST candidate Pydantic validation (side=HOLD) → 422
- approve happy: PENDING → APPROVED → EXECUTED_PAPER + virtual_order_id /
  VirtualOrder status=CREATED 영속 (실 KIS 호출 0건)
- approve unknown 404, status=EXECUTED_PAPER 후 재호출 409, kill_switch=True 503
- reject happy + new_status=REJECTED + reason 영속 / empty reason 422
- expire happy + new_status=EXPIRED / 이미 EXECUTED_PAPER → 409
- audit log timeline (CREATED → RISK_CHECKED → APPROVED → EXECUTED_PAPER) /
  event_type filter / unknown event 422
- forbidden 응답 substring 17종 (`api_key / token / secret / source_file_path /
  broker_order_id / kis_order_id / real_account / real_order_id / broker_name /
  account_number / raw_text / body / full_text / access_token / jwt_secret /
  kis_app_secret / kis_account_no`) 0건 — 모든 응답 단언
- AST 검사 — `approval_routes.py` / `approval_service.py` 가 `requests / httpx /
  urllib / urllib3 / app.kis / app.data.dart_provider / app.data.rss_provider /
  app.data.collectors.kis_client` import 0건
- read-only 405 가드 parametrize 6건 (PUT/DELETE/PATCH on candidates / audit)

### 6.46.3 Approval scheduler job — `tests/integration/test_approval_scheduler_jobs.py` (6건)

- trading_safety=False → SKIPPED + reason `trading_safety_disabled`
- kill_switch=True → SKIPPED + reason `kill_switch_enabled`
- 활성화 + overdue 0 → SUCCESS + reason `nothing_expired`
- 활성화 + overdue 2 + fresh 1 → SUCCESS, expired_count=2, fresh 보존,
  audit `EXPIRED` 1행씩 영속
- run_job 래퍼 → JobOutcome.status=SUCCESS + JobRun row 1건
- `DEFAULT_INTERVAL_SCHEDULE[expire_pending_approvals] == 5` 등록 단언

### 6.46.4 회귀 가드 갱신

- `test_alembic_migration.py`: `HEAD_REVISION='0008_approval_audit_logs'`,
  `EXPECTED_TABLE_COUNT=39`, spot-check + parametrize 에 `approval_audit_logs` 추가
- `test_auth_security.py`: mutating endpoint count 11 → 15
  (auth 2 + watchlist 6 + preferences 1 + paper 2 + **approval 4**).
  `no_auto_trade_strings_in_routes` 가드는 `approval` keyword 를
  `/api/approvals/` prefix 만 허용 (그 외는 거부)
- `test_scheduler_jobs.py`: registry sanity 11 → 12 jobs
- `test_safety_settings.py`: Phase C 가드 → Phase D 가드 (Alembic 8 revisions
  정확히, approval 모듈 3종 **존재** 단언)
- `test_scheduler_module.py`: `DEFAULT_INTERVAL_SCHEDULE` 인지하도록 3 테스트
  갱신 (cron+interval 합집합 / interval 잡 cron 검증 skip /
  `interval_schedule={}` 명시 override)

### 6.46.5 회귀 기준

- backend pytest **1622 → 1693 passed (+71)** — 회귀 0건
- frontend vitest **186 passed** — 변경 없음 (Phase D 는 backend only)
- frontend build 그린; e2e **22 passed** — 변경 없음
- Alembic head `0008_approval_audit_logs` (신규 1건); `compare_metadata` diff 0건
- mutation 엔드포인트 11 → 15 (+approval 4); read-only 라우터 +3
  (candidates list / detail / audit). 실 KIS / FULL_AUTO / SMALL_AUTO 라우터 0건 유지
- ScoringEngine / HoldingCheckEngine weight 변경 0건; 신규 pip 0건;
  KIS / 외부 네트워크 호출 0건; 평문 IP / user_agent 저장 0건;
  forbidden 응답 / details 필드 0건

---

## 6.47 v0.15 Phase E — Approval UI 14번째 화면 + 마감 문서 테스트

### 6.47.1 신규 vitest (15건)

`frontend/src/tests/Approvals.test.tsx` (15 tests):

- page render — `approvals-page` / `approvals-policy-banner` /
  `approvals-new-form` / `approvals-new-submit` 라벨 "후보 만들기 (Risk Check)"
- empty placeholders — 후보 0건 시 `approvals-pending-empty` /
  `approvals-history-empty`
- pending table — `approvals-pending-table` + `approvals-row-{id}` +
  `approvals-approve-{id}` ("승인 (paper 실행)") / `approvals-reject-{id}` /
  `approvals-expire-{id}` 버튼 라벨
- 503 disabled banner (general) — POST /approve 503 시
  `approvals-disabled-banner` 출현 / `approvals-approve-success` 부재
- 503 kill switch banner — `detail` 에 "kill switch" 포함 시
  `approvals-kill-switch-banner` 출현
- approve happy path — POST /approve 성공 → `approvals-approve-success` 에
  virtual_order_id 노출
- reject prompt-driven reason — `window.prompt` mock → POST /reject body 의
  `reason` 일치
- expire 호출 — POST /expire 호출 카운트
- detail drawer — row 클릭 → `approvals-detail-drawer` /
  `approvals-detail-summary` / `approvals-detail-risk` /
  `approvals-detail-audit` + audit row 노출
- risk violation rendering — RISK_REJECTED + violations[] 시
  `approvals-detail-violation-{rule_id}` 노출
- history table — REJECTED status query → `approvals-history-table` +
  `approvals-history-row-{id}`
- new candidate disabled banner — POST /candidates 503 시
  `approvals-new-disabled-banner`
- new candidate success — POST /candidates 성공 시 `approvals-new-success`
- forbidden DOM 토큰 0건 — `api_key / access_token / jwt_secret /
  kis_app_secret / source_file_path / broker_order_id / kis_order_id /
  real_account / real_order_id / account_number / raw_text / full_text` 0건
- 자동매매 CTA 0건 — `자동매매 시작 / 자동매매 모드 / 실거래 시작 /
  실거래 모드 / FULL_AUTO / SMALL_AUTO / place real order` 0건

### 6.47.2 신규 e2e (1건)

`frontend/e2e/dashboard.spec.ts`:

- `Approvals page surfaces policy banner / pending table / new-candidate form,
  never leaks KIS fields` — `/approvals` 진입 → `approvals-page` 가시 →
  policy banner / empty placeholders 검증 → submit 라벨 "후보 만들기
  (Risk Check)" → forbidden DOM 토큰 12종 0건 → automation actionable CTA
  6종 0건 → POST /candidates 503 시 `approvals-new-disabled-banner` 출현 →
  raw payload (`/api/approvals/candidates` + `/audit`) 에 forbidden
  substring 0건
- `all 14 sidebar menus are reachable and render their main content` —
  사이드바 13 → 14 갱신 (`승인 대기 (β)` 메뉴 추가)

### 6.47.3 신규 msw / e2e fixture mock (7건)

`frontend/src/tests/mswServer.ts`:

- GET `/api/approvals/candidates` → `{ candidates: [], total: 0, limit: 50 }`
- GET `/api/approvals/candidates/:id` → 404
- GET `/api/approvals/audit` → `{ items: [], total: 0, limit: 50 }`
- POST `/api/approvals/candidates` / `/:id/approve` / `/:id/reject` /
  `/:id/expire` → 503 disabled

`frontend/e2e/fixtures/apiMocks.ts`:

- 위 7 패턴 + 503 default — happy-path 테스트는 `server.use(...)` 로 override.

### 6.47.4 회귀 기준

- backend pytest **1693 passed** (Phase E 는 frontend only 이므로 backend
  변경 0건)
- frontend vitest **186 → 201 passed (+15)** — Approvals.test 신규
- frontend build 그린; e2e **22 → 23 passed (+1)** — `/approvals` happy-path
- Alembic / API / DB / pip 변경 0건; KIS / FULL_AUTO / SMALL_AUTO 라우터 0건 유지
- forbidden 응답 / DOM 필드 0건; automation CTA 0건; ScoringEngine weight
  변경 0건

---

## 6.48 v0.16 Phase A — RealTradingSettings + safety defaults 테스트

### 6.48.1 대상 파일

- `tests/unit/test_safety_settings.py` — v0.16 섹션 8~13 (신규 60 건)
- `tests/unit/test_project_structure.py` — 5 신규 default 단언

### 6.48.2 paranoid defaults 검증 (섹션 8)

- 5 종 신규 필드 전부 동시 단언 (`test_v016_real_trading_defaults_are_paranoid`)
- 개별 격리 단언 3 건: `real_trading_enabled=False` / `kis_order_enabled=False` /
  `real_order_dry_run=True`

### 6.48.3 env override 검증 (섹션 8)

- `REAL_TRADING_ENABLED` — 10 개 토큰 parametrize (true/True/1/on/YES/false/False/0/off/NO)
- `KIS_ORDER_ENABLED` — 6 개 토큰 parametrize
- `REAL_ORDER_DRY_RUN` — 4 개 토큰 parametrize
- `MAX_REAL_ORDER_AMOUNT` / `MAX_REAL_DAILY_ORDER_AMOUNT` — 수치 override 1 건

### 6.48.4 strict-bool typo 안전 default 검증 (섹션 9)

- `REAL_TRADING_ENABLED` typo → `False` 유지 (6 typo parametrize)
- `KIS_ORDER_ENABLED` typo → `False` 유지 (3 typo)
- `REAL_ORDER_DRY_RUN` typo → `True` 유지 (3 typo) — dry-run 이 절대 silently OFF 되지 않음

### 6.48.5 boundary validation 검증 (섹션 10)

- `max_real_order_amount <= 0` → `ValueError("must be > 0")` (3 bad value parametrize)
- `max_real_daily_order_amount <= 0` → `ValueError` (3 bad value parametrize)
- `max_real_order_amount > max_real_daily_order_amount` → `ValueError` (cross-validation)
- `max_real_order_amount == max_real_daily_order_amount` → 허용 (edge case)

### 6.48.6 can_attempt_real_order_settings() helper 검증 (섹션 11)

- 기본 default → `False` (실거래 진입 구조적 불가)
- 6 gate 전부 활성화 → `True`
- 6 gate 중 하나씩 닫았을 때 → `False` (parametrize 6 케이스)

### 6.48.7 Phase A 격리 단언 (섹션 12)

- Alembic revision 정확히 8 개 (`0001` ~ `0008`) — 0009/0010 미존재 단언
- `real_order.py` / `real_fill.py` 미존재 — Phase C 아티팩트
- `kis_order_client.py` 미존재 — Phase B 아티팩트

### 6.48.8 v0.15 회귀 단언 (섹션 13)

- `kill_switch_enabled=True` 기본 유지
- `trading_safety_enabled=False` 기본 유지
- `paper_trading_enabled=False` 기본 유지

### 6.48.9 회귀 기준

- backend pytest **1693 → 1753 passed (+60)** — 회귀 0건
- Alembic revision 0건 / DB 모델 변경 0건 / API 라우터 변경 0건 / 프런트 변경 0건
- KIS API / 외부 네트워크 호출 0건 / 실 KIS 주문 코드 0건
- `real_trading_enabled=False` / `kis_order_enabled=False` / `real_order_dry_run=True` 기본 유지

---

## 6.49 v0.16 Phase B — KIS Order Wrapper Skeleton + FakeKisOrderTransport 테스트

### 6.49.1 대상 파일

- `tests/unit/test_kis_order_client.py` — 신규 35 건
- `tests/unit/test_safety_settings.py` — Phase A guard 1 건 제거 (`kis_order_client.py` 미존재 단언)

### 6.49.2 ABC 인터페이스 단언 (섹션 1)

- `KisOrderClientInterface()` 직접 인스턴스화 → `TypeError` (abstractmethod 존재)

### 6.49.3 KisOrderRequest 불변성 + 마스킹 (섹션 2–3)

- 속성 setattr → `AttributeError`
- `repr()` / `as_dict()` — `account_no` 마스킹 확인 (실 값 비노출, last-4 suffix + `****` 존재)

### 6.49.4 KisOrderRequest 입력 검증 (섹션 4)

- bad side (5 케이스 parametrize) → `ValueError("side")`
- bad order_type (4 케이스) → `ValueError("order_type")`
- quantity 0 / 음수 → `ValueError("quantity")`
- price 음수 → `ValueError("price")`
- MARKET + price=0 → 허용

### 6.49.5 KisFillStatusResult status 검증 (섹션 5)

- 알 수 없는 status 문자열 → `ValueError("status")`

### 6.49.6 result dataclass as_dict 정확성 (섹션 6)

- `KisOrderResult.as_dict()` → 3 키 검증
- `KisFillStatusResult.as_dict()` → 6 키 / `PARTIALLY_FILLED` 상태
- `KisCancelResult.as_dict()` → 3 키 검증

### 6.49.7 FakeKisOrderTransport 동작 (섹션 7–9)

- `place_order` → `success=True` + `FAKE-XXXXXX` 패턴 order_no
- 연속 2회 호출 시 order_no 단조 증가 확인
- `query_fill_status` → `status="FILLED"`
- `cancel_order` → `success=True` + order_no 일치

### 6.49.8 mask_sensitive_order_payload (섹션 10)

- `api_key` / `api_secret` / `account_no` → `"***MASKED***"` 치환
- 대소문자 무관 마스킹 (`API_KEY`, `Authorization`)
- 원본 dict 불변 확인

### 6.49.9 __init__.py re-export 단언 (섹션 11)

- `from app.broker import KisOrderClientInterface, FakeKisOrderTransport, ...` 7종 import 성공

### 6.49.10 AST 가드 (섹션 12)

- `httpx` / `requests` / `urllib` import 0건 단언 (AST walk)
- `app.data.collectors.kis` import 0건 단언

### 6.49.11 Phase C / D scope 가드 (섹션 13)

- `real_order.py` / `real_fill.py` 미존재 — Phase C 아티팩트
- `real_order_executor.py` 미존재 — Phase D 아티팩트

### 6.49.12 회귀 기준

- backend pytest **1753 → 1787 passed (+34)** — 회귀 0건
- Alembic revision 0건 / DB 모델 변경 0건 / API 라우터 변경 0건 / 프런트 변경 0건
- KIS API / 외부 네트워크 호출 0건 / 실 KIS 주문 코드 0건

---

## 6.50 v0.16 Phase C — RealOrder / RealFill ORM + Alembic + Repository 테스트

### 6.50.1 대상 파일

- `tests/integration/test_real_order_repository.py` — 신규 48 건
- `tests/integration/test_real_fill_repository.py` — 신규 23 건
- `tests/integration/test_alembic_migration.py` — HEAD / COUNT / spot-check 갱신
- `tests/unit/test_safety_settings.py` — Phase A scope guard 갱신 (real_order.py 존재 허용)
- `tests/unit/test_kis_order_client.py` — Phase B scope guard 갱신

### 6.50.2 RealOrder Repository 테스트 (test_real_order_repository.py)

**create() 검증:**
- MARKET / LIMIT 주문 생성 성공
- dry_run=True 기본값 확인
- bad side / zero quantity / LIMIT without price / MARKET with price → ValueError

**상태 전이:**
- CREATED → SUBMITTED 전이 허용
- terminal 상태 (DRY_RUN / FILLED / CANCELED / REJECTED / FAILED) → 재전이 불가 (5케이스 parametrize)

**mark_submitted:**
- broker_order_no_hash + submitted_at 기록 확인

**mark_failed 민감값 가드:**
- error_message에 `api_key` / `appsecret` / `access_token` / `account_no` 포함 → ValueError (4케이스)
- 500자 초과 → 자동 truncate

**mark_dry_run:**
- CREATED → DRY_RUN 전이 성공; 타 상태에서 호출 → ValueError

**Cascade delete:**
- OrderCandidate 삭제 시 연결된 RealOrder 자동 삭제

**Forbidden columns (11종 parametrize):**
...
**AST 가드 (3종):**
- httpx / requests / urllib import 0건

**No raw-response method:**
- `dir(RealOrderRepository)` 에서 `raw` 포함 메서드 0건

### 6.50.3 RealFill Repository 테스트 (test_real_fill_repository.py)

**create() 검증:**
- FULL / PARTIAL fill 생성 성공
- unknown fill_status / zero quantity / zero fill_price / negative gross_amount → ValueError

**list_by_order / list_recent:**
- 복수 fill 목록 조회 / 최신 순 정렬

**Cascade delete:**
- RealOrder 삭제 시 연결 RealFill 자동 삭제
...
**No raw-response method:**
- `dir(RealFillRepository)` 에서 `raw` 포함 메서드 0건

### 6.50.4 Alembic 마이그레이션 갱신

- `HEAD_REVISION = "0010_real_fills"` (구 `0008_approval_audit_logs`)
- `EXPECTED_TABLE_COUNT = 41` (구 39)
- `SPOT_CHECK_TABLES` 에 `real_orders` / `real_fills` 추가
- `compare_metadata` diff 0건 유지 (server_default 불일치 없음)

### 6.50.5 회귀 기준

- backend pytest **1787 → 1858 passed (+71)** — 회귀 0건
- Alembic head `0010_real_fills` / 누적 테이블 41 개 / compare_metadata diff 0건
- API 라우터 변경 0건 / 프런트 변경 0건
- KIS API / 외부 네트워크 호출 0건 / 실 KIS 주문 코드 0건
- raw KIS response 저장 0건 / 계좌번호·API key·token·secret 저장 0건

---

## 6.51 v1.0 Phase A — Real Trading Operating Checklist + Final Safety Gates 테스트

### 6.51.1 대상 파일

- `RUNBOOK_REAL_TRADING.md` (신규, repo root) — §1~9 운영 절차 / 운영 체크리스트 / v1.0 금지 사항
- `app/config/settings.py` 갱신 — `RECOMMENDED_MAX_REAL_*_KRW` 모듈 상수 2종 +
  `validate_real_trading_operating_limits(settings) -> list[str]` 순수 advisory helper
  (신규 Settings 필드 추가 0건)
- `tests/unit/test_safety_settings.py` v1.0 Phase A 단위 테스트 +38 건 (§14~§19)

### 6.51.2 RUNBOOK_REAL_TRADING.md 검증 (§14)

- `_runbook_path()` helper 로 repo root `/RUNBOOK_REAL_TRADING.md` 경로 단언
- 17 종 필수 키워드 parametrize (`pytest.mark.parametrize`):
  - env-var name 8 종: `REAL_TRADING_ENABLED` / `KIS_ORDER_ENABLED` / `REAL_ORDER_DRY_RUN` /
    `KILL_SWITCH_ENABLED` / `APPROVAL_REQUIRED` / `TRADING_SAFETY_ENABLED` /
    `MAX_REAL_ORDER_AMOUNT` / `MAX_REAL_DAILY_ORDER_AMOUNT`
  - 섹션 앵커 9 종: `§1` ~ `§9`
- 정책 잠금 라인 (§9):
  - `❌ **FULL_AUTO` 명시 금지 라인 존재
  - `❌ **SMALL_AUTO` 명시 금지 라인 존재
  - `❌ **사용자 승인 없는 주문**` 라인 존재
  - `❌ **실거래 default ON**` 라인 존재

### 6.51.3 paranoid default 재검증 (§15)

- v0.15 + v0.16 + v0.14 + v0.8 layer 일관 — env 12 변수 모두 `delenv` 후 `Settings()` 직접 생성:
  - v0.15: `trading_safety_enabled=False` / `kill_switch_enabled=True` / `approval_required=True`
  - v0.16: `real_trading_enabled=False` / `kis_order_enabled=False` / `real_order_dry_run=True`
  - v0.14: `paper_trading_enabled=False`
  - v0.8: `auth_enabled=False`
  - `can_attempt_real_order_settings(s) is False` 단언
- `max_real_order_amount=100_000` / `max_real_daily_order_amount=1_000_000` 기본값 단언

### 6.51.4 validate_real_trading_operating_limits() advisory helper (§16)

- paranoid defaults (100k / 1M) → 빈 list 반환 (advisory 0 건)
- `max_real_order_amount > RECOMMENDED_MAX_REAL_ORDER_AMOUNT_KRW (1,000,000)` → advisory 1 건
- `max_real_daily_order_amount > RECOMMENDED_MAX_REAL_DAILY_ORDER_AMOUNT_KRW (10,000,000)` → advisory 1 건
- 두 cap 모두 초과 → advisory 2 건 (안정 순서: per-order 먼저, daily 다음)
- pure 함수 — 동일 Settings 두 번 호출 시 동일 list 반환 (no log / no I/O / no side-effect)
- 정확히 임계값 = 권장 cap 일 때 advisory 0 건 (boundary inclusive)

### 6.51.5 can_attempt_real_order_settings() × RUNBOOK §2 일관성 (§17)

- 6 게이트 모두 open → True
- 6 게이트 각각 1 개씩 closed (parametrize 6 케이스) → 모두 False

### 6.51.6 Phase B scope guard (§18)

Phase B 진입 전 다음 4 종 모두 미존재:
- `app/broker/kis_order_transport_real.py` 파일 부재
- `app.broker.kis_order_client` 모듈에 `HttpxKisOrderTransport` 심볼 부재
- `app.broker.real_order_executor` 모듈에 `HttpxKisOrderTransport` /
  `execute_real` / `real_path_execute` 심볼 부재
- `app.broker.fill_sync_service` 모듈에 `sync_fills_real` /
  `HttpxKisOrderTransport` 심볼 부재

### 6.51.7 Alembic / pyproject 보존 (§19)

- Alembic head 그대로 — 10 revisions (`0001_baseline_v0_7.py` ~ `0010_real_fills.py`)
  unchanged. v1.0 Phase A 추가 revision 0 건
- `pyproject.toml` 에 `httpx` / `respx` 보존 (v0.11 dev-only respx, 기존 production httpx)
- 외부 broker SDK (`ccxt` / `broker-sdk` / `kis-sdk` / `openapi-kis`) 미추가 단언

### 6.51.8 회귀 기준

- backend pytest **1905 → 1942 passed (+38)** — 회귀 0 건
  (로컬 `MARKET_CAP_LIMIT=5` `.env` 차이로 1 건 deselect 는 무관)
...
## 6.54 v1.0 Phase D — FillSyncService 델타 idempotent + POST /sync API 테스트

### 6.54.1 대상 파일

- `app/broker/fill_sync_service.py` 갱신 — v0.16 mock-only → v1.0 델타 기반 idempotent + 6 분류 +
  audit + DRY_RUN skip
- `app/data/repositories/real_fill.py` 갱신 — `total_filled_quantity(real_order_id)` 헬퍼 추가
- `app/data/repositories/approval_audit_log.py` 갱신 — `VALID_EVENT_TYPES` 에 3 종 추가
  (`REAL_ORDER_FILL_SYNCED` / `REAL_ORDER_FILL_FAILED` / `FILL_SYNC_NEGATIVE_DELTA`)
- `app/api/real_order_routes.py` 갱신 — `POST /api/real-orders/{order_id}/sync` 신규 mutation +
  `_require_trading_safety_enabled` / `_require_kill_switch_off` helpers
- `app/api/schemas.py` 갱신 — `RealOrderSyncRequest` + `RealOrderSyncResponse` 신규
- `tests/unit/test_fill_sync_service.py` 24 신규 + 기존 갱신
- `tests/integration/test_fill_sync_integration.py` (신규) 7 건
- `tests/integration/test_real_order_sync_api.py` (신규) 10 건
- `tests/unit/test_safety_settings.py` Phase D scope guard 전환 5 종
- `tests/integration/test_auth_security.py` mutating endpoint count 15 → 16

### 6.54.2 6-class 분류 + 매핑

| KIS 응답 | classification | RealOrder.status 전이 | 비고 |
|---|---|---|---|
| `success=True, status=FILLED` | FULL | → FILLED | delta = max(filled_quantity, total_qty) |
| `success=True, status=PARTIALLY_FILLED` | PARTIAL | → PARTIALLY_FILLED | delta = filled_quantity |
| `success=True, status=PENDING` | NONE | (변경 없음) | delta = 0 |
| `success=False, status=PENDING` | FAILED | (변경 없음) | UNKNOWN-mapped (transport-layer) |
| `success=False, status=REJECTED` | REJECTED | → REJECTED | broker 거부 |
| `success=False, status=CANCELED` | CANCELED | → CANCELED | broker 취소 |
| transport raise | FAILED | (변경 없음) | audit `REAL_ORDER_FILL_FAILED` |

### 6.54.3 델타 idempotency
...
delta == 0 → 신규 행 0건 + audit REAL_ORDER_FILL_SYNCED (idempotent re-call)
delta < 0 → audit FILL_SYNC_NEGATIVE_DELTA + return FAILED, skipped_reason="NEGATIVE_DELTA"
```

`_effective_kis_total()` 정책:
- FULL → `max(filled_quantity, total_qty)` — FakeKisOrderTransport 가 filled_quantity=0 으로
  FILLED 응답할 때도 정상 동작 보장
- PARTIAL → `filled_quantity`
- NONE/REJECTED/CANCELED/FAILED → 0

### 6.54.4 DRY_RUN skip 정책
...
실 broker order_no 와 충돌할 수 있는 위험을 차단. 단언 테스트 1 건이 `_BoomTransport` 로 transport
호출 0건을 강제 검증.

### 6.54.5 plaintext kis_order_no 정책

`sync_fills(session, real_order_id, *, kis_order_no_plaintext=None)` 의 옵션 파라미터로 운영자가
보유한 KIS 평문 주문번호를 in-memory 만 transport 에 전달. 단언:
- `RealFill.symbol/side/fill_status/fill_price/gross_amount` 어디에도 plaintext 미저장
- `ApprovalAuditLog.reason` / `details_json` 에 plaintext 미포함
- `RealOrderSyncResponse` 응답 본문에서 plaintext echo 0건 (HTTP 통합 테스트로 검증)

### 6.54.6 ApprovalAuditLog 연동
...
- audit details whitelist (모두 forbidden 13 종 미포함):
  ...
  - `dry_run` (boolean — Phase D 는 항상 False, DRY_RUN 은 audit 미작성)
  - `symbol` / `side`
  - `delta` / `existing_total` / `kis_total` (negative-delta audit 만)
  - `kis_status` (FAILED audit 만)
- `test_phase_d_audit_details_have_no_forbidden_keys` 가 모든 audit row details key 가 forbidden
  13 종에 0건 단언

### 6.54.7 POST /api/real-orders/{order_id}/sync API

- 게이트: AUTH (`require_auth`) + `TRADING_SAFETY_ENABLED=true` + `KILL_SWITCH_ENABLED=false`
- `REAL_TRADING_ENABLED` / `KIS_ORDER_ENABLED` 미강제 (사고 후 sync 가능 정책 — RUNBOOK §6 기준)
- 응답 `RealOrderSyncResponse` 7 필드 (real_order_id / real_order_status / fill_status / fills_added /
  fills_total / synced_at / message). 응답 본문 forbidden substring (api_key / app_secret /
  access_token / kis_account_no / kis_app_key / kis_app_secret / broker_order_no / broker_order_id /
  kis_order_id / real_account / raw_text / raw_response / full_text / source_file_path / jwt_secret) 0건
- 에러 코드: 200 / 401 (auth_required + missing Bearer) / 404 (order not found) / 405 (PUT/PATCH/DELETE) /
  503 (TRADING_SAFETY_DISABLED 또는 KILL_SWITCH_ON)
- 통합 테스트 10 건 (TestClient + dependency overrides)

### 6.54.8 mutating endpoint count

15 → **16** (real-orders 1 추가). `test_mutating_endpoint_count_unchanged` parametrize 정확
일치 단언. `_ALLOWED_ORDER_PREFIXES` 의 `/api/real-orders` 가 신규 mutation 도 포함 (변경 0 건).

### 6.54.9 AST + import 가드

- `fill_sync_service.py` 직접 `httpx.Client` 0 건 / `httpx` import 0 건 / `requests` 0 건 /
  `urllib` 0 건 / `KisHttpOrderTransport` 직접 reference 0 건
- transport 는 항상 DI (`KisOrderClientInterface` 인자로 주입) — production 에서 `HttpxKisOrderTransport`
  연결은 운영자 책임

### 6.54.10 회귀 기준

- backend pytest **2040 → 2082 passed (+42)** — 회귀 0 건
- Alembic head: `0010_real_fills` 변경 0 건 / 41 테이블 그대로
- DB 모델 변경 0 건 / 프런트 변경 0 건
- 신규 pip 의존성 0 건 — `httpx` / `respx` / stdlib 만
- 실 KIS API 호출 0 건 / 외부 네트워크 호출 0 건
- API key / app_secret / access_token / account_number / plaintext order_no 평문 저장·로그·응답 0 건
- raw KIS response 저장·반환 0 건
