...
> 누적 cycle 의 게이트 baseline 과 v0.4–v0.14 신규 테스트 카테고리를 반영한다.

## 1. 현재 회귀 게이트 (v0.14 마감 시점)
...
| 게이트 | 명령 | 현재 baseline |
|---|---|---|
| backend pytest | `python -m pytest -q` | **1438 passed** (v0.14 누적: 1277 → 1438, +161 — Phase A 45 + Phase B 43 + Phase C 40 + Phase D 33; Phase E 는 frontend only 회귀 0건) |
| frontend vitest | `cd frontend && npm run test -- --run` | **186 passed** (v0.14 Phase E: 175 → 186, +11 — `PaperTrading.test.tsx` 11 신규: page wrapper / account happy / 404 empty / positions / pnl / orders / disabled 503 / submit success / cancel mutation / forbidden DOM 토큰 0건 / 자동매매 CTA 0건) |
| frontend build | `cd frontend && npm run build` | 그린 (`tsc --noEmit && vite build`) |
| Playwright e2e | `cd frontend && npm run e2e` | **22 passed** (v0.14 Phase E: 21 → 22, +1 — `/paper` happy-path navigation + raw payload forbidden 검사 + 503 disabled banner) |
