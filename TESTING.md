...
> 본 문서는 **v0.14 Phase C 시점** 기준으로 갱신되었다 (`v0.14-pnl-tracker`
> 태그 예정). 누적 cycle 의 게이트 baseline 과 v0.4–v0.14 신규 테스트 카테고리를
> 반영한다.

## 1. 현재 회귀 게이트 (v0.14 Phase C 완료 시점)
...
| 게이트 | 명령 | 현재 baseline |
|---|---|---|
| backend pytest | `python -m pytest -q` | **1405 passed** (v0.14 Phase C: 1365 → 1405, +40 — paper cost model 8건 + virtual PnL engine 27건 + alembic migration 추가 + Phase B placeholder 단언 1건 제거 흡수) |
| frontend vitest | `cd frontend && npm run test -- --run` | **175 passed** (v0.13 Phase D: 165 → 175, +10 — v0.14 Phase C 미변경) |
| frontend build | `cd frontend && npm run build` | 그린 (`tsc --noEmit && vite build`) |
| Playwright e2e | `cd frontend && npm run e2e` | **21 passed** (v0.14 Phase C 미변경) |
