...
> 본 문서는 **v0.10 마감 시점** 기준으로 갱신되었다 (마감 태그 `v0.10-final`).
> v0.10 **Real Provider Readiness & Resilience** 사이클 완료 —
> `ProviderHealthMonitor` + `call_with_resilience()` (Phase A) + DART
> Provider skeleton (Phase B) + RSS/News Provider skeleton (RSS 2.0 + Atom,
> stdlib only) (Phase C) + `GET /api/health/providers` + Provider Health
> Settings 패널 (Phase D). 모든 provider 는 transport 주입형 skeleton 으로,
> 실 httpx 전송 도입은 **v0.11+ 라이선스 검토(사람) 선행 후** 별도 cycle.
...
v0.10 마감 후 진입 가능 후보. 현재는 모두 미착수 — 명시적 진입 요청 전까지 손대지 않는다.

> v0.10 에서 채택된 항목 (provider resilience runtime / DART provider skeleton /
> RSS provider skeleton / Provider Health API + UI) 은 이 목록에서 제거됨.

### 9.1 데이터 / 분석 실제화 (라이선스 검토 선행 필수)

- **DART / RSS 실 httpx transport 도입** — v0.10 의 transport 주입형 skeleton
  위에 실 HTTP 전송 + retry policy 튜닝. **DART OpenAPI 이용 약관 + 상업 목적
  가부 + Rate limit 정책을 운영자(사람)가 확인 후 진입.** RSS 도 동일 — 운영자가
  허가받은 feed URL 만.
- **DART / RSS score 반영** — `RealNewsScoreProducer` (v0.5) /
  `RealFundamentalScoreProducer` (v0.6) 위에 DART / RSS 데이터 연결. 룰 기반
  검증 후 ScoringEngine weight 보강.
...
- **Prometheus exporter + Grafana 대시보드** — v0.10 의 `ProviderHealthMonitor`
  in-memory state 를 외부 시계열 DB 로 노출. v0.9 Sentry optional 위에 메트릭
  레이어 추가. 외부 노출 규모 확인 후 진입.
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v0.11+) DART/RSS 실 httpx transport / Prometheus / Grafana / 인증 고도화 / 백테스트 고도화 / LLM 보강 / DART/RSS score 반영
