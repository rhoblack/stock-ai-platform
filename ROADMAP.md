...
v0.9 마감 후 진입 가능 후보. 현재는 모두 미착수 — 명시적 진입 요청 전까지 손대지 않는다.

> v0.9 에서 채택된 항목 (rate limit / security headers / Sentry / UserPreference / Watchlist API 고도화) 은 이 목록에서 제거됨.

### 9.1 데이터 / 분석 실제화 (라이선스 검토 선행 필수)

- **실 DART API 구현체** — v0.6 의 `FundamentalProviderInterface` / `EarningsProviderInterface` ABC 위에 `DartFundamentalProvider` / `DartEarningsProvider` 추가. **DART OpenAPI 이용 약관 + 상업 목적 가부 + Rate limit 정책을 운영자가 확인 후 진입** — v0.9 provider_status + retry interface 는 이 시점을 위한 준비.
...
- **Prometheus exporter + Grafana 대시보드** — v0.9 Sentry optional 위에 메트릭 레이어 추가. 외부 노출 규모 확인 후 진입.
...
| Operational Security & Watchlist Polish | rate limit + security headers + brute force + Sentry + 구조화 로깅 + Watchlist API 고도화 + UserPreference + provider 회복성 | **v0.9 ⏳ 진행 중** — v0.8 위에 운영 필수 보안 + UX 고도화. 자동매매 / 실 Provider 구현 0건 |
...
`BrokerInterface` 는 ABC placeholder 만 유지. v0.1 ~ v0.9 어디에도 구현체 없음.
...
목록 관리 (rename/delete/default/memo) 만 — 자동매매 / 브로커 0건.
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v0.9 ⏳) rate limit + security headers + Sentry + Watchlist API 고도화 + UserPreference + provider 회복성
   → (v0.10+) 실 DART/RSS provider / Prometheus / Grafana / 인증 고도화 / 백테스트 고도화 / LLM 보강
