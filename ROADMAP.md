...
> 본 문서는 **v0.11 시작 시점** 기준으로 갱신되었다 (시작 태그 `v0.10-final`,
> 마감 예정 태그 `v0.11-final`). v0.10 **Real Provider Readiness & Resilience**
> 마감 후, v0.11 **Real Provider Transport & Observability** 5 phase 진입.
> 채택 시나리오: **Scenario X — Real Provider Transport + Observability**
> (DART/RSS 실 httpx transport + observability ring buffer + Prometheus
> exporter optional + `/api/health/providers` 확장). DART/RSS/Prometheus 모두
> default OFF 유지. 자동매매 / MockBroker / FULL_AUTO 는 여전히 **Future
> Backlog**. Score 반영 / 백테스트 고도화 / 인증 고도화 / LLM 보강은 v0.12+
> 후보로 연기.
...
| v0.11 Real Provider Transport & Observability | DART/RSS 실 httpx transport (default OFF 유지) + provider observability (failure history ring buffer + summary_24h + optional Prometheus `/metrics`) + `/api/health/providers` 확장 + Settings 패널 보강 | ⏳ 진행 중 | `v0.11-final` (예정) |
...
## v0.11 — Real Provider Transport & Observability (⏳ 진행 중)

기준선: `v0.10-final`. 회귀 게이트: pytest 1045 (1 deselected) / vitest 153 /
e2e 20 / build 그린. **Alembic head 변경 없음** (`0004_user_preferences` 그대로,
provider observability 도 in-memory bounded ring buffer).

채택 시나리오: **Scenario X — Real Provider Transport + Observability**.

| Phase | 작업 | 상태 | 예상 태그 |
|---|---|---|---|
| A | DART HTTP Transport — `HttpxDartTransport` + respx mock 테스트 ~18건 | ⏳ 대기 | `v0.11-dart-transport` |
| B | RSS HTTP Transport — `HttpxRssTransport` + respx + RSS 2.0/Atom fixture ~12건 | ⏳ 대기 | `v0.11-rss-transport` |
| C | Provider Observability Layer — failure history ring buffer + summary_24h + optional Prometheus `/metrics` ~20건 | ⏳ 대기 | `v0.11-observability` |
| D | `/api/health/providers` 확장 + Settings 패널 보강 (success_rate_24h / recent_failures) +8 / +6 / +1 | ⏳ 대기 | `v0.11-health-extended` |
| E | 마감 — `RELEASE_NOTES_v0.11.md` + 문서 갱신 + 태그 `v0.11-final` | ⏳ 대기 | `v0.11-final` |
...
- Alembic 새 revision 0건 — provider observability bounded ring buffer 만
- 본문 / 비밀값 / URL query secret 응답·로그·UI 평문 노출 0건
- 신규 pip 의존성 2종: `respx>=0.21,<0.22` (테스트 only) + `prometheus-client>=0.19,<1.0`

상세 계획: [`PLANS.md`](./PLANS.md) `PLAN-0011`

> **자동매매 / 실주문 (FULL_AUTO / SMALL_AUTO / BrokerInterface 구현)** 는 v0.11 에도
...
v0.11 마감 후 진입 가능 후보. 현재는 모두 미착수 — 명시적 진입 요청 전까지 손대지 않는다.

> v0.11 에서 채택된 항목 (DART/RSS 실 httpx transport / provider observability /
> Prometheus exporter / `/api/health/providers` 확장) 은 이 목록에서 제거됨.

### 9.1 데이터 / 분석 실제화

- **DART / RSS score 반영** — v0.11 의 실 transport 위에 `RealNewsScoreProducer`
  (v0.5) / `RealFundamentalScoreProducer` (v0.6) 가 DART/RSS 데이터를 흡수.
...
| Real Provider Transport & Observability | DART/RSS 실 httpx transport (default OFF 유지) + observability ring buffer + summary_24h + Prometheus exporter optional + `/api/health/providers` 확장 | **v0.11 ⏳ 진행 중** — provider 가 production-ready. DART/RSS score 반영 / 백테스트 고도화 / 인증 고도화 / LLM 보강은 v0.12+ |
...
`BrokerInterface` 는 ABC placeholder 만 유지. v0.1 ~ v0.10 어디에도 구현체 없음.
v0.7 의 `StrategySignal` 도 분석 신호이지 매매 주문이 아니다 — broker / 주문 /
계좌 / 가격 / 수량 / order_type / side 필드 0건. v0.8 의 Watchlist `POST` 도
즐겨찾기 추가/삭제만 — 주문 / Broker / 자동매매 0건. v0.9 의 PUT/DELETE 라우터도
목록 관리 (rename/delete/default/memo) 만 — 자동매매 / 브로커 0건. v0.10 의
`GET /api/health/providers` 는 read-only — Provider toggle / mutation 0건
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v0.11 ⏳) DART/RSS 실 httpx transport + provider observability + Prometheus exporter optional + /api/health/providers 확장
   → (v0.12+) Grafana / DART/RSS score 반영 / 백테스트 고도화 / 인증 고도화 / LLM 보강
