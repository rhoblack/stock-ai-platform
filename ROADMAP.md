...
> 본 문서는 **v0.6 마감 선언 시점** 기준으로 갱신되었다 (직전 마감 태그
> `v0.6-frontend-fundamentals`, 최종 마감 태그 `v0.6-final` 예정). 초안 (자동매매
...
> 뉴스·공시 데이터 라인 + `news_score` 첫 real 화 + 테마 랭킹 화면 + 재무·실적
> 데이터 라인 + `fundamental_score` / `earnings_score` 첫 real 화 + StockDetail
> Fundamental·Earnings 카드 중심이다.
...
`BrokerInterface` 는 ABC placeholder 만 유지. v0.1 ~ v0.6 어디에도 구현체 없음.
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v0.7+) 인증 / Watchlist / 글로벌 검색 / DART 실 API / LLM 보강 / Alembic
