...
| Approval Trading Safety Layer | OrderCandidate (38번째) + PreTradeRiskEngine (6 hard 룰) + SafetySettings (KILL_SWITCH=true 기본) + Approval Workflow API 7종 + ApprovalAuditLog (39번째, append-only) + 14번째 화면 `/approvals` (paper execution only) | **v0.15 ⏳ 진행 중** — 실 KIS 주문 0건. 승인된 후보는 SimulationBroker 만 호출. Approval Trading / 실거래 연결은 v0.16+ |
...
| Real Order Integration (v0.16+) | KIS order wrapper + 실주문 audit log + 실주문 실패 핸들러. v0.15 OrderCandidate(status=APPROVED) → KisOrderClient.place_order(candidate) | 컴플라이언스 / 자본 한도 / 비상정지 / 실주문 로그 보존 별도 사이클 선행 + KIS API 라이선스 검토 + 운영자 책임 / 면책 정책 명문화 |
| APPROVAL 모드 (실거래) | 사용자 승인 후 실 KIS 주문 — 비상정지 / 일일 손실 제한 / 종목당 한도 / 전략별 한도 / 실거래 로그 / 보안 강화 동반 | v0.16 Real Order Integration 안정 운영 후 |
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v0.15 ⏳) OrderCandidate + PreTradeRiskEngine (6 hard 룰) + SafetySettings (KILL_SWITCH=true 기본) + Approval Workflow API + ApprovalAuditLog (append-only) + 14번째 화면 /approvals — paper execution only
   → (v0.16+) Real Order Integration: KIS order wrapper + 실주문 audit (별도 컴플라이언스 / 자본 한도 사이클 선행 후)
   → (v0.17+) ScoringEngine weight 보강 (walk-forward 검증 후) / Grafana dashboard / monitor 영속화 / 인증 고도화 / LLM 보강 / WebSocket
   → (Future) ReplayBroker / 스트레스 테스트 → APPROVAL 실거래 → SMALL_AUTO
```
