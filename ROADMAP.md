...
> 본 문서는 **v1.0 마감 시점** 기준으로 갱신되었다 (마감 태그 `v1.0-final`, 2026-05-09).
> v1.0 채택 시나리오: **Scenario X — Small Approval Trading Release (소액·승인 기반 첫 실 KIS 주문)**.
> v0.16 의 RealTradingSettings + KIS Order Wrapper Skeleton + RealOrder/RealFill ORM
> (Alembic `0010_real_fills`, 41 테이블) + RealOrderExecutor (8-gate dry-run 전용) +
> FillSync mock 위에 **`HttpxKisOrderTransport` (실 KIS 주문 transport, 첫 실 구현체) +
> RealOrderExecutor real path (10-gate, dry-run vs real 분기) + idempotent Fill Sync (델타 기반)** 도입.
> Alembic revision 추가 0건 (기존 schema 재사용). **사용자 승인 없는 주문 0건 / FULL_AUTO / SMALL_AUTO
> 0건 유지 (본 프로젝트 명시 금지 정책)** — 1 후보 = 1 사용자 승인 = 1 실주문 원칙.
> 실 KIS 호출은 운영자가 `.env` 에서 `REAL_TRADING_ENABLED=true` + `KIS_ORDER_ENABLED=true` +
> `REAL_ORDER_DRY_RUN=false` 모두 명시 활성화 시에만, 모든 테스트는 `respx` mock 100%.
> Reconciliation 은 v1.1 이연.
...
| v1.0 Small Approval Trading Release | `HttpxKisOrderTransport` (KisOrderClientInterface 첫 실 구현체) + RealOrderExecutor real path (10-gate, dry-run vs real 분기) + idempotent Fill Sync (델타 기반, Alembic 0건) + `POST /api/real-orders/{id}/sync` 1 mutation + 15번째 화면 강화 (RealTradingModeBanner / Sync Fill 버튼) + RUNBOOK_REAL_TRADING + 운영 진입 체크리스트 | ✅ 마감 | `v1.0-final` |
...
기준선: `v0.12-final`. 최종 게이트: **pytest 1277 / vitest 175 / e2e 21 / build
그린**. **Alembic head 변경 0건** (`0004_user_preferences` 그대로 — score_delta 는
기존 `evidence_json` JSON 컬럼 재활용; Validation Report 는 기존 `backtest_runs` /
`backtest_results` 테이블 쿼리). **마감 일자: 2026-05-08. 마감 태그: `v0.13-final`.**

채택 시나리오: **Scenario X — Provider Score Policy + Score Delta + Validation
Report + Backtest Export CLI** (Backtest Export CLI 는 v0.14 Phase A 로 이연).

| Phase | 작업 | 상태 | 태그 |
|---|---|---|---|
| A | ProviderScorePolicy Engine — `app/scoring/provider_policy.py` + `DATA_SOURCE_RELIABILITY` dict + `apply_policy(score, data_source)` + `PROVIDER_SCORE_POLICY_ENABLED=False` 기본 + FAKE=bypass + 회귀 단언 ~25건 | ✅ 인수 | `v0.13-provider-policy` |
| B | Score Delta in evidence_json — `score_before`/`score_after`/`delta`/`components[{name, data_source, factor, before, after}]` JSON 필드 (Alembic 0건) + 단위 테스트 ~15건 | ✅ 인수 | `v0.13-score-delta` |
| C | Validation Report Read-only API — `GET /api/validation/report` (전체 summary) + `/by-strategy` + `/by-regime` + `/by-sector` + 통합 테스트 ~15건 | ✅ 인수 | `v0.13-validation-api` |
| D | Validation Report UI + Score Delta UI 확장 — React 화면 + vitest ~7건 + Validation 12번째 화면 (ClipboardCheck 아이콘, staleTime 60_000) | ✅ 인수 | `v0.13-validation-ui` |
| E | 마감 — `RELEASE_NOTES_v0.13.md` + 문서 갱신 + 4 게이트 최종 확인 (Backtest Export CLI 는 v0.14 Phase A 이연) | ✅ 문서 마감 | `v0.13-final` |
...
상세 계획: [`PLANS.md`](./PLANS.md) `PLAN-0013` / 마감 사유: [`RELEASE_NOTES_v0.13.md`](./RELEASE_NOTES_v0.13.md)
...
채택 시나리오: **Scenario X — Approval Trading Safety Layer (paper execution only)**.
(비교 후보: Y=Approval UI 중심, Z=KIS 실주문 wrapper 준비, W=Paper Trading 고도화)
...
- **`KILL_SWITCH_ENABLED=true` 기본 (paranoid)** — 명시 false 없이는 모든 후보 차단
...
상세 계획: [`PLANS.md`](./PLANS.md) `PLAN-0015`

> **v0.16+ 경계**: v0.15 의 `OrderCandidate(status=APPROVED)` /
> `PreTradeRiskEngine` / `ApprovalAuditLog` / `KillSwitch` 는 v0.16 의 직접
> 기반이 된다. v0.16 진입 전에는 **컴플라이언스 / 자본 한도 정책 / 비상정지
> 이력 / 실주문 로그 보존** 의 별도 사이클이 선행되어야 한다. v0.15 자체는
> paper execution 만이 종착점이며 실 KIS 주문 / 실 broker 0건 정책을 유지한다.

---

## v0.16 — Real Order Integration Skeleton & Fill Sync Readiness (✅ 마감)

기준선: `v0.15-final` (1693 / 201 / 23 / build) → 마감: `v0.16-final` (**1905 / 214 / 24 / build**,
+212 pytest / +13 vitest / +1 e2e). **Alembic 신규 2건** — `0009_real_orders` (RealOrder, Phase C) +
`0010_real_fills` (RealFill, Phase C, 누적 41 테이블).
**`REAL_TRADING_ENABLED=false` / `KIS_ORDER_ENABLED=false` / `REAL_ORDER_DRY_RUN=true`
모두 paranoid default** — 실 KIS 주문 v0.16 전체에서 0건.

채택 시나리오: **Scenario X (핵심) + Scenario Y Phase 1 (RealOrder/RealFill ORM)**.
(비교: Y=Fill Sync 중심 — Fill Sync mock 만 Phase D 포함; Z=실 KIS 주문 — 기각;
W=Reconciliation — v1.1 이연)

| Phase | 작업 | 상태 | 태그 |
|---|---|---|---|
| A | RealTradingSettings 5 종 (`REAL_TRADING_ENABLED=false` / `KIS_ORDER_ENABLED=false` / `REAL_ORDER_DRY_RUN=true` / `MAX_REAL_ORDER_AMOUNT=100k` / `MAX_REAL_DAILY_ORDER_AMOUNT=1M`) + 단위 테스트 +60 건 | ✅ 인수 | `v0.16-real-trading-settings` |
| B | `KisOrderClientInterface` ABC + `FakeKisOrderTransport` + AST 가드 + 마스킹 단언 +34 건 | ✅ 인수 | `v0.16-kis-order-wrapper` |
| C | `RealOrder` ORM (40 번째) + `RealFill` ORM (41 번째) + Alembic `0009` + `0010` + Repository 2 종 + 통합 테스트 +71 건 | ✅ 인수 | `v0.16-real-order-orm` |
| D | `RealOrderExecutor` (8-gate dry-run 전용) + `FillSyncService` (mock transport 주입형) + 단위 +47 건 | ✅ 인수 | `v0.16-real-order-executor` |
| E | 15 번째 화면 `/real-orders` (read-only, `TrendingUp` 아이콘, SafetyBanner / DryRunAttemptsTable) + `RELEASE_NOTES_v0.16.md` + 4 게이트 최종 확인 | ✅ 문서 마감 | `v0.16-final` |
...
- **실 KIS 주문 0건** — v0.16 전체 cycle 에서 AST 회귀 단언으로 검증
...
- **Alembic 2 revisions** (`0009_real_orders` + `0010_real_fills`, 누적 41 테이블)
- **Reconciliation 0건** — v1.1 이연
- **FULL_AUTO / SMALL_AUTO / 자동매매 0건** — v0.1~v0.16 일관
- **신규 pip 의존성 0건** — stdlib + 기존 의존성만

상세 계획: [`PLANS.md`](./PLANS.md) `PLAN-0016`

> **v1.0 경계**: v0.16 의 `RealOrder` / `RealFill` ORM + `KisOrderClientInterface` +
> `RealOrderExecutor` (dry-run) + `FillSyncService` (mock) 가 v1.0 의 **직접 기반** 이 된다.
> v1.0 은 `HttpxKisOrderTransport` (KisOrderClientInterface 첫 실 구현체) + RealOrderExecutor real path
> + idempotent Fill Sync 를 도입하는 **Small Approval Trading Release** 사이클이다.

---

## v1.0 — Small Approval Trading Release (✅ 마감)

기준선: `v0.16-final` (1905 / 214 / 24 / build, HEAD `2d262d5`) → 마감: `v1.0-final`
(**2082 / 225 / 25 / build**, +177 pytest / +11 vitest / +1 e2e).
**Alembic revision 추가 0건** — 기존 schema (`0010_real_fills`, 41 테이블) 재사용.
Idempotency 는 RealFill 델타 비교 기반. mutating endpoint count: 15 → **16**
(`POST /api/real-orders/{id}/sync` 1 추가).
**`REAL_TRADING_ENABLED=false` / `KIS_ORDER_ENABLED=false` / `REAL_ORDER_DRY_RUN=true` /
`MAX_REAL_ORDER_AMOUNT=100k` / `MAX_REAL_DAILY_ORDER_AMOUNT=1M` 모두 paranoid default 그대로 유지** —
실 KIS 주문은 운영자가 `.env` 에서 명시 활성화 + 사용자 명시 승인 시에만 1 건씩 진입.

채택 시나리오: **Scenario X — Small Approval Trading Release**.
(비교: Y=KIS transport + dry-run 유지·실주문 v1.1 이연 — 기각 (transport 만으로는 v1.0 가치 부족);
Z=Reconciliation 까지 포함 — Reconciliation 은 v1.1 로 이연하고 Phase D 에는 idempotent Fill Sync 만
포함하는 형태로 부분 흡수; W=SMALL_AUTO 까지 — 기각 (본 프로젝트 명시 금지 정책 위반))

| Phase | 작업 | 상태 | 태그 | 실측 게이트 |
|---|---|---|---|---|
| A | Real trading operating checklist (`RUNBOOK_REAL_TRADING.md` § 1~9) + safety gates 보강 + settings `validate_real_trading_operating_limits()` advisory helper + 단위 테스트 +38 건 | ✅ 인수 | `v1.0-operating-checklist` | pytest 1905→1942 |
| B | `HttpxKisOrderTransport` (KisOrderClientInterface 첫 실 구현체) + httpx lazy + timeout (5/10/5s) + retry 정책 (place=0 / query·cancel=2) + `mask_sensitive_order_payload` + broker_order_no SHA-256 + `respx` mock 100% +53 건 | ✅ 인수 | `v1.0-kis-real-transport` | pytest 1942→1995 |
| C | `RealOrderExecutor` real path (10-gate: 기존 8 + TRADING_SAFETY_DISABLED + TRANSPORT_UNAVAILABLE) + dry-run vs real 분기 + `RealOrder(dry_run=False, status=CREATED→SUBMITTED)` 저장 + `_resolve_real_transport()` + `_execute_real()` + audit (REAL_ORDER_SUBMITTED/FAILED) + 단위 +37 건 + 통합 +8 건 | ✅ 인수 | `v1.0-real-order-executor-real` | pytest 1995→2040 |
| D | `FillSyncService` 실 transport 연결 + 6 분류 (FULL/PARTIAL/NONE/REJECTED/CANCELED/FAILED) + 델타 기반 idempotent RealFill (Alembic 0건) + DRY_RUN skip + `POST /api/real-orders/{id}/sync` 신규 mutation (KillSwitch+TradingSafety+AUTH 3중 게이트) + audit (REAL_ORDER_FILL_SYNCED/FAILED + FILL_SYNC_NEGATIVE_DELTA) + 단위 +24 + 통합 +18 | ✅ 인수 | `v1.0-fill-sync-real` | pytest 2040→2082 |
| E | 15 번째 화면 `/real-orders` 강화 (`RealTradingModeBanner` 5 flag badge + 4 mode badge + 수동 `Sync Fill` 버튼, KillSwitch ON 일 때 disabled) + `useSyncRealOrder` mutation hook + Settings 5 bool 노출 + RELEASE_NOTES_v1.0 + RUNBOOK_REAL_TRADING 최종 + vitest +11 / e2e +1 + 4 게이트 최종 확인 | ✅ 문서 마감 | `v1.0-final` | vitest 214→225 / e2e 24→25 |

**v1.0 핵심 정책 (cycle-wide)**:

- **paranoid default 9종 유지** — `REAL_TRADING_ENABLED=false` / `KIS_ORDER_ENABLED=false` /
...
- **소액 / 일일 한도 강제** — `MAX_REAL_ORDER_AMOUNT` (회당) + `MAX_REAL_DAILY_ORDER_AMOUNT` (일일 누적)
...
- **raw KIS response 저장 0건** — `RealOrder` / `RealFill` 모든 필드에서 금지
- **mutation 라우터 1 건 신규** — `POST /api/real-orders/{id}/sync` 만 (KillSwitch + TradingSafety + AUTH 3중 게이트)
- **Alembic revision 0건** — 기존 schema 재사용 (델타 기반 idempotency)
- **신규 pip 의존성 0건** — `respx` (v0.11 dev-only) / `httpx` (기존) 만
- **Reconciliation 0건** — v1.1 이연
- **자동 폴링 잡 0건** — Fill Sync 는 수동 트리거만. 자동 폴링은 v1.1
- **FULL_AUTO / SMALL_AUTO / 자동매매 0건** — 본 프로젝트 명시 금지 정책 그대로
- **DART/RSS/Prometheus/Provider Data Ingestion/Score Policy/Paper Trading default OFF 유지**

상세 계획: [`PLANS.md`](./PLANS.md) `PLAN-0017`

> **v1.1+ 경계**: v1.0 의 `HttpxKisOrderTransport` + `RealOrderExecutor` real path + idempotent Fill Sync
> 가 v1.1 의 **직접 기반** 이 된다. v1.1 에서는 Reconciliation (`reconciliation_runs` /
> `reconciliation_mismatches` 신규 테이블, Alembic 1~2건) + 자동 Fill Sync 폴링 잡 + PreTradeRiskEngine
> 신규 룰 (호가 jump / 시장 시간 외 차단 / 종목당 일일 한도 / 연속 실패 차단) 도입. **SMALL_AUTO /
> FULL_AUTO 는 v1.1 에서도 0건 유지** — 본 프로젝트 명시 금지 정책.

---

## v1.1+ 후보

v1.0 마감 후 진입 가능 후보. 현재는 모두 미착수 — 명시적 진입 요청 전까지 손대지 않는다.

> v0.14~v0.16 에서 채택된 항목 (SimulationBroker + Paper Trading / Approval Trading Safety Layer /
> Real Order Integration Skeleton + RealOrder·RealFill ORM + dry-run RealOrderExecutor + FillSync mock)
> 과 v1.0 에서 채택된 항목 (HttpxKisOrderTransport + RealOrderExecutor real path + idempotent Fill Sync)
> 은 이 목록에서 제거됨.

### 1.1 Reconciliation + 자동 Fill Sync (v1.1 핵심)

- **Reconciliation 엔진** — 내부 `RealOrder` / `RealFill` vs KIS 잔고/체결 비교. mismatch 분류 (수량 / 평단 /
  미체결 / 누락) + 발생 시 KillSwitch 자동 활성화 옵션. `reconciliation_runs` / `reconciliation_mismatches`
  신규 테이블 (Alembic 1~2건). 진입 전제: v1.0 운영 4 주 + 실주문 ≥10 건
- **자동 Fill Sync 폴링 잡** — `SUBMITTED` 상태 RealOrder 가 N 분 이상 갱신 없으면 자동 sync.
  `KIS_ORDER_ENABLED=true` 시에만 동작
- **PreTradeRiskEngine 신규 룰** — 호가 jump 검사 / 시장 시간 외 차단 / 종목당 일일 한도 / 연속 실패 종목 차단
- **`submitted_at` / `filled_at` timestamps + idempotency key** — `RealOrder` 에 timestamp 2종 + `kis_request_id_hash` 추가
  (Alembic `0011_real_order_v1_fields`, 옵션)
...
### 1.6 백테스트 고도화 (v0.7~v0.9 데이터 누적 후 단계적)
...
| Provider Score Policy & Validation Report | ProviderScorePolicy 승수 엔진 (weight 변경 0건) + score_delta in evidence_json + Validation Report GET API (Backtest Export CLI v0.14 이연) | **v0.13 ✅ 마감** — data_source provenance 를 policy engine 으로 연결 완료. ScoringEngine weight 직접 보강은 v0.15+ |
| SimulationBroker + Paper Trading Foundation | Backtest Export CLI 선처리 + SimulationBroker (`BrokerInterface` 첫 구현체) + VirtualAccount/Order/Position/Fill/PnL 5 테이블 (Alembic 2건) + Paper Trading API/UI | **v0.14 ✅ 마감** — 실 KIS 주문 0건, PAPER_TRADING_ENABLED=False 기본 |
| Approval Trading Safety Layer | OrderCandidate (38번째) + PreTradeRiskEngine (7 hard 룰) + SafetySettings (KILL_SWITCH=true 기본) + Approval Workflow API 7종 + ApprovalAuditLog (39번째, append-only) + 14번째 화면 `/approvals` (paper execution only) | **v0.15 ✅ 마감** — 실 KIS 주문 0건. 승인된 후보는 SimulationBroker 만 호출. |
| Backtest 엔진 고도화 | Grid Search 튜닝, hyperparameter 최적화, 시장 국면별 성과 세분 분석 | v0.15 Approval workflow 안정 후 |
| MockBroker / ReplayBroker | 시장 데이터 재생 기반 replay, 스트레스 테스트 | SimulationBroker (v0.14) 안정 운영 후 |
| 전용 ML 모델 | Market Regime / Strategy Selection / Risk Prediction. 추천 성과 기반 재학습. LLM 과 전용 AI 역할 분리 | Backtest 데이터 누적 후행 |
| Real Order Integration Skeleton (v0.16) | RealTradingSettings (5 종, paranoid defaults) + KIS Order Wrapper Skeleton (FakeKisOrderTransport) + RealOrder/RealFill ORM (40·41 번째) + RealOrderExecutor (dry-run 전용) + FillSync mock + 15 번째 화면 `/real-orders` | **v0.16 ✅ 마감** — 실 KIS 주문 0건. 모든 실행은 dry-run. Reconciliation 은 v1.1 이연 |
| Small Approval Trading Release (v1.0) | `HttpxKisOrderTransport` + RealOrderExecutor real path (10-gate) + idempotent Fill Sync (델타 기반, Alembic 0건) + `POST /api/real-orders/{id}/sync` mutation 1건 + RealTradingModeBanner + 수동 Sync Fill 버튼 + RUNBOOK_REAL_TRADING + 운영 진입 체크리스트 | **v1.0 ✅ 마감** (2026-05-09, `v1.0-final`) — `KIS_ORDER_ENABLED=true` + `REAL_ORDER_DRY_RUN=false` 최초 활성화 가능. 사용자 명시 승인된 후보만 실 KIS 주문 (소액 / 일일 한도 / 회당 한도 / 중복 가드) — 1 후보 = 1 승인 = 1 주문 원칙. backend 2082 / vitest 225 / e2e 25 / build green |
| Reconciliation + 자동 Fill Sync 폴링 (v1.1) | 내부 RealOrder/RealFill vs KIS 잔고/체결 비교 + mismatch 감지 + KillSwitch 자동 활성화 + `reconciliation_runs` / `reconciliation_mismatches` 신규 테이블 (Alembic 1~2건) + 자동 Fill Sync 폴링 잡 + PreTradeRiskEngine 신규 룰 (호가 jump / 시장 시간 외 / 종목당 일일 한도 / 연속 실패 차단) | v1.0 안정 운영 4 주 + 실주문 ≥10 건 + mismatch 0 건 또는 분류 가능 누적 후 |
| SMALL_AUTO | 소액 자동 — 본 프로젝트 명시 금지 정책. 별도 보안·컴플라이언스 사이클 + 사용자 승인 정책 재검토 선행 필수 | 본 프로젝트 범위 외 |
| FULL_AUTO | 전면 자동 — 본 프로젝트 명시 금지 정책 | 본 프로젝트 범위 외 — 별도 설계 cycle |
...
`BrokerInterface` 는 v0.1 ~ v0.13 까지 ABC placeholder 만 유지 (구현체 없음). v0.14 에서
`SimulationBroker` 가 **첫 구현체**로 추가되지만 **실 KIS 주문 0건** (가상 시뮬레이션만).
v0.7 의 `StrategySignal` 도 분석 신호이지 매매 주문이 아니다 — broker / 주문 / 계좌 /
가격 / 수량 / order_type / side 필드 0건. v0.8 의 Watchlist `POST` 도 즐겨찾기
추가/삭제만 — 주문 / Broker / 자동매매 0건. v0.9 의 PUT/DELETE 라우터도 목록 관리
(rename/delete/default/memo) 만. v0.10 의 `GET /api/health/providers` 는 read-only
(POST/PUT/DELETE 모두 405). v0.11 의 `GET /metrics` (Prometheus) 도 read-only —
default OFF 시 404. v0.12 의 walk-forward / 다중 전략 결과는 기존
`backtest_runs.notes` JSON 재활용 (Alembic revision 0건, 새 라우터 모두 GET only).
v0.13 의 Validation Report API 도 GET only (POST→405 단언). v0.14 의
`POST /api/paper/orders` 는 가상 주문만 — 실 KIS broker 호출 0건. v0.15 의
`POST /api/approvals/...` mutation 은 KillSwitch + TradingSafety + AUTH 3중
게이트 + 승인된 후보의 실행조차 `SimulationBroker` 만 호출 — 실 KIS 주문 0건
유지.
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v0.13 ✅) ProviderScorePolicy 승수 엔진 + score_delta in evidence_json + Validation Report GET API 4종 + 12번째 화면 Validation Report UI
   → (v0.14 ✅) Backtest Export CLI + SimulationBroker (BrokerInterface 첫 구현체) + VirtualAccount/Order/Position/Fill/PnL + Paper Trading API/UI 13번째 화면
   → (v0.15 ✅) OrderCandidate + PreTradeRiskEngine (7 hard 룰) + SafetySettings (KILL_SWITCH=true 기본) + Approval Workflow API + ApprovalAuditLog (append-only) + 14번째 화면 /approvals — paper execution only
   → (v0.16 ✅) RealTradingSettings (5 종, paranoid) + KIS Order Wrapper Skeleton (FakeKisOrderTransport) + RealOrder/RealFill ORM (40·41번째, Alembic 2건) + RealOrderExecutor (dry-run 전용, 8-gate) + FillSync mock + 15번째 화면 /real-orders — 실 KIS 주문 0건
   → (v1.0 ✅) Small Approval Trading Release — HttpxKisOrderTransport + RealOrderExecutor real path (10-gate) + idempotent Fill Sync (델타 기반) + POST /sync 1 mutation + RealTradingModeBanner + 수동 Sync Fill 버튼 + RUNBOOK_REAL_TRADING — KIS_ORDER_ENABLED=true + REAL_ORDER_DRY_RUN=false 최초 활성화 가능, 사용자 명시 승인된 후보만 실 KIS 주문 (소액 / 일일 한도 / 회당 한도 / 중복 가드)
   → (v1.1) Reconciliation (`reconciliation_runs` / `reconciliation_mismatches` 신규) + 자동 Fill Sync 폴링 잡 + PreTradeRiskEngine 신규 룰 (호가 jump / 시장 시간 외 / 종목당 일일 한도 / 연속 실패) + ScoringEngine weight 보강 / Grafana / monitor 영속화 / 인증 고도화 / LLM 보강 / WebSocket
   → (Future) ReplayBroker / 스트레스 테스트. SMALL_AUTO / FULL_AUTO 는 본 프로젝트 명시 금지 정책 (본 프로젝트 범위 외)
```
