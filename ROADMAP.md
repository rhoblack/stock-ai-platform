...
> 본 문서는 **v1.0 시작 시점** 기준으로 갱신되었다 (기준 태그 `v0.16-final`, HEAD `2d262d5`).
...
| v1.0 Small Approval Trading Release | `HttpxKisOrderTransport` (KisOrderClientInterface 첫 실 구현체) + RealOrderExecutor real path (10-gate, dry-run vs real 분기) + idempotent Fill Sync (델타 기반, Alembic 0건) + `POST /api/real-orders/{id}/sync` 1 mutation + 15번째 화면 강화 (RealTradingModeBanner / Sync Fill 버튼) + RUNBOOK_REAL_TRADING + 운영 진입 체크리스트 | ⏳ 진행 중 | `v1.0-final` (목표) |
...
## v1.0 — Small Approval Trading Release (⏳ 진행 중)

기준선: `v0.16-final` (1905 / 214 / 24 / build, HEAD `2d262d5`). **Alembic revision 추가 0건** —
기존 schema (`0010_real_fills`, 41 테이블) 재사용. Idempotency 는 RealFill 델타 비교 기반.
**`REAL_TRADING_ENABLED=false` / `KIS_ORDER_ENABLED=false` / `REAL_ORDER_DRY_RUN=true` /
...
| Phase | 작업 | 상태 | 예상 태그 |
|---|---|---|---|
| A | Real trading operating checklist (`RUNBOOK_REAL_TRADING.md` § 1~7: 활성화·비상중지·KillSwitch·주문실패·체결불일치·dry-run rollback·릴리스) + safety gates 보강 + settings `__post_init__` 강화 + 단위 테스트 +10 건 | ⏳ 대기 | `v1.0-operating-checklist` |
| B | `HttpxKisOrderTransport` (KisOrderClientInterface 첫 실 구현체) + httpx lazy + timeout (5/10/5s) + retry 정책 (place=0 / query·cancel=2) + `mask_sensitive_order_payload` + broker_order_no SHA-256 + `respx` mock 100% +30 건 | ⏳ 대기 | `v1.0-kis-real-transport` |
| C | `RealOrderExecutor` real path (10-gate: 기존 8 + DUPLICATE_REAL_ORDER + TRANSPORT_UNAVAILABLE) + dry-run vs real 분기 + `RealOrder(is_dry_run=False, status=PENDING→SUBMITTED)` 저장 + audit 1 행 + 단위 25 + 통합 10 | ⏳ 대기 | `v1.0-real-order-executor-real` |
| D | `FillSyncService` 실 transport 연결 + 5 분류 (FULL/PARTIAL/NONE/REJECTED/CANCELED) + 델타 기반 idempotent RealFill (Alembic 0건) + `POST /api/real-orders/{id}/sync` 신규 mutation (KillSwitch + TradingSafety + AUTH 3중 게이트) + 단위 25 + 통합 20 | ⏳ 대기 | `v1.0-fill-sync-real` |
| E | 15 번째 화면 `/real-orders` 강화 (RealTradingModeBanner / Sync Fill 버튼, KillSwitch ON 일 때 disabled) + RELEASE_NOTES_v1.0 + RUNBOOK_REAL_TRADING 최종 + vitest +5 / e2e +1 + 4 게이트 최종 확인 | ⏳ 대기 | `v1.0-final` |
...
| Small Approval Trading Release (v1.0) | `HttpxKisOrderTransport` (KisOrderClientInterface 첫 실 구현체) + RealOrderExecutor real path (10-gate, dry-run vs real 분기) + idempotent Fill Sync (델타 기반, Alembic 0건) + `POST /api/real-orders/{id}/sync` mutation 1건 (KillSwitch+TradingSafety+AUTH 3중 게이트) + RUNBOOK_REAL_TRADING + 운영 진입 체크리스트 | **v1.0 ⏳ 진행 중** — `KIS_ORDER_ENABLED=true` + `REAL_ORDER_DRY_RUN=false` 최초 활성화 가능. 사용자 명시 승인된 후보만 실 KIS 주문 (소액 / 일일 한도 / 회당 한도 / 중복 가드) — 1 후보 = 1 승인 = 1 주문 원칙 |
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v1.0 ⏳) Small Approval Trading Release — HttpxKisOrderTransport (KisOrderClientInterface 첫 실 구현체) + RealOrderExecutor real path (10-gate, dry-run vs real 분기) + idempotent Fill Sync (델타 기반, Alembic 0건) + POST /api/real-orders/{id}/sync 1 mutation + RUNBOOK_REAL_TRADING — KIS_ORDER_ENABLED=true + REAL_ORDER_DRY_RUN=false 최초 활성화 가능, 사용자 명시 승인된 후보만 실 KIS 주문 (소액 / 일일 한도 / 회당 한도 / 중복 가드)
