...
| v0.8 User & Migration Foundation | Alembic baseline (27 → 31 테이블) + 단일 사용자 인증 (`AUTH_ENABLED` 토글 + JWT) + Watchlist 도메인 GET/POST/DELETE (POST 라우터 첫 도입) + Watchlist 프런트 (11번째 화면) + StockDetail/Today 즐겨찾기 통합 | ⏳ 진행 중 (2026-05-06 시작) | `v0.8-final` (예정) |
...
phase 누적 예정 — v0.1 부터 일관 유지된 read-only 정책의 **첫 변경 cycle**.
27 테이블 + 누적 ALTER 5건 시점에 **Alembic baseline 도입** + 단일 사용자
인증 기반 (`AUTH_ENABLED` 토글 + JWT + `LoginAuditLog`) 위에 **Watchlist
도메인 POST/DELETE 첫 도입**. ScoringEngine / HoldingCheckEngine 본 weight
변경 0건, 자동매매 / 실 KIS / DART / Telegram 호출 0건 정책 그대로. 마감
게이트 (예상): backend pytest **~775 / 84 → ~95 / 14 → 16 / build 그린**.

| Phase | 작업 | 상태 | 태그 (예정) |
|---|---|---|---|
| A | Alembic baseline (`alembic.ini` + `env.py` + `0001_baseline_v0_7.py` 27 테이블 + CI 검증 + INTEGRATION_RUNBOOK §17) | ⏳ 대기 | `v0.8-alembic-baseline` |
| B | 단일 사용자 인증 (`User` 28 + `LoginAuditLog` 29 + JWT + `AUTH_ENABLED` 토글 + `POST /api/auth/login` 첫 POST) | ⏳ 대기 | `v0.8-auth-foundation` |
| C | Watchlist DB / API (`Watchlist` 30 + `WatchlistItem` 31 + GET/POST/DELETE 4 라우터 + 인증 가드) | ⏳ 대기 | `v0.8-watchlist-api` |
| D | Watchlist 프런트 + Today/StockDetail 통합 (11번째 화면 + `/login` + Sidebar `관심종목 ★`) | ⏳ 대기 | `v0.8-frontend-watchlist` |
| E | `RELEASE_NOTES_v0.8.md` + 마감 + tag `v0.8-final` | ⏳ 대기 | `v0.8-final` |
...
| User & Auth Foundation | 단일 사용자 인증 + Watchlist + Alembic baseline + POST 첫 도입 | **v0.8 ⏳ 진행 중** — POST 라우터 = 5건 한정 (auth 3 + watchlist 2). 다중 사용자 / OAuth / RBAC 0건 |
