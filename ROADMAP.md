...
| SimulationBroker + Paper Trading Foundation | Backtest Export CLI 선처리 + SimulationBroker (`BrokerInterface` 첫 구현체) + VirtualAccount/Order/Position/Fill/PnL 5 테이블 (Alembic 2건) + Paper Trading API/UI | **v0.14 ⏳ 진행 중** — 실 KIS 주문 0건, PAPER_TRADING_ENABLED=False 기본. Approval Trading / 실거래 연결은 v0.15+ |
| Backtest 엔진 고도화 | Grid Search 튜닝, hyperparameter 최적화, 시장 국면별 성과 세분 분석 | v0.14 Paper Trading 검증 후 |
...
`POST /api/paper/orders` 는 가상 주문만 — 실 KIS broker 호출 0건.
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v0.15+) ScoringEngine weight 보강 (walk-forward 검증 후) / Grafana dashboard / monitor 영속화 / 인증 고도화 / LLM 보강 / WebSocket
   → (Future) ReplayBroker / 스트레스 테스트 → APPROVAL → SMALL_AUTO
```
