...
> 본 문서는 **v0.7 진입 선언 시점** 기준으로 갱신되었다 (v0.6 마감 태그
> `v0.6-final` 부여 완료, v0.7 시작점). 초안 (자동매매 중심 v0.2~v1.0 구상) 과
> 실제 진행 이력이 달라져 전면 재작성. 자동매매 / MockBroker / FULL_AUTO 는
> 모두 **Future Backlog** 로 이동했고, 실제로 진행된 사이클은 분석·UX·CI·
> 운영 정착 + 증권사 리포트 인텔리전스 + 뉴스·공시 데이터 라인 + `news_score`
> 첫 real 화 + 테마 랭킹 화면 + 재무·실적 데이터 라인 + `fundamental_score` /
> `earnings_score` 첫 real 화 + StockDetail Fundamental·Earnings 카드 중심이다.
> v0.7 부터는 **Strategy / Backtest 기초** 가 진입 — 추천 판단의 사후 검증 +
> 자동매매 진입 전 필수 단계.
...
| v0.7 Strategy & Backtest Foundation | `StrategyInterface` + 룰 기반 전략 2~3종 + `BacktestEngine` (recommendation_results 1·3·5·20일 활용) + 비용 모델 (placeholder 0.45% 차감) + 시장 국면별 분리 + 백테스트 화면 (10번째) | 🟡 Phase A 진입 대기 | `v0.7-strategy-interface` (Phase A 예정) |
...
## v0.7 — Strategy & Backtest Foundation (🟡 Phase A 진입 대기)

기준선: `v0.6-final` (HEAD `e729d60`). 558 / 77 / build / 13 baseline 위에 5 phase
진행 — v0.1~v0.6 누적된 추천 판단 축 (technical / report / theme / news /
disclosure / fundamental / earnings + risk_penalty + recommendation_results
1·3·5·20일) 위에 **`StrategyInterface` ABC + 룰 기반 전략 + `BacktestEngine` +
비용 모델 + 시장 국면별 분리 + 백테스트 화면** 을 도입한다. 다음 자연 질문
"이 추천이 돈이 되는가?" 에 답하기 위한 cycle.

| Phase | 작업 | 상태 | 태그 (예정) |
|---|---|---|---|
| A | Strategy interface — `StrategyInterface` ABC + `TopGradeStrategy` / `HighScoreStrategy` / `MultiSignalStrategy` 3종 + 단위 테스트 | ⏳ 진입 대기 | `v0.7-strategy-interface` |
| B | Backtest engine — `BacktestRun` (26번째 테이블) + `BacktestResult` (27번째 테이블) + `BacktestEngine` + `scripts/run_backtest.py` (default dry-run) | ⏳ | `v0.7-backtest-engine` |
| C | 비용 모델 + 시장 국면별 분리 — `CostModel` (placeholder buy 0.015% / sell 0.015% / 거래세 0.20% / 슬리피지 0.1% = 약 0.45% 차감) + `MarketRegimeRepository` 활용 | ⏳ | `v0.7-backtest-cost-regime` |
...
| E | `RELEASE_NOTES_v0.7.md` + 마감 선언 + 4 게이트 재확인 | ⏳ | `v0.7-final` |
...
상세 계획: [`PLANS.md`](./PLANS.md) `PLAN-0007`

---

## v0.8+ 후보

v0.7 마감 후 진입 가능 후보. 현재는 모두 미착수 — 명시적 진입 요청 전까지 손대지 않는다.

### 8.1 인증 / 관심종목 (v0.8 우선 후보)
...
- **Alembic 도입 + 마이그레이션 자동화** — 누적 ALTER 가 v0.5 `news_items.category` + v0.6 신규 테이블 2개 + v0.7 신규 테이블 2개 = 5건 시점에 도입 적기.
...
| 단계 | 범위 | 진입 전제 |
|---|---|---|
| Strategy & Signal | StrategyInterface 활성화, SIGNAL 모드 + 룰 기반 전략 + 백테스트 검증 | v0.7 Strategy & Backtest Foundation (🟡 진행 중) — 실 News (v0.5 ✅) + 재무 / 어닝 데이터 (v0.6 ✅) 위에 직접 진입 (인증 없이도 read-only 검증 가능) |
...
`BrokerInterface` 는 ABC placeholder 만 유지. v0.1 ~ v0.7 어디에도 구현체 없음.
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v0.7 진행) Strategy / Backtest 기초 + 비용 모델 + 백테스트 화면
   → (v0.8+) 인증 / Watchlist / Alembic / DART 실 API / LLM 보강 / 운영 모니터링
