...
> 본 문서는 **v0.16 시작 시점** 기준으로 갱신되었다 (기준 태그 `v0.15-final`).
> v0.16 채택 시나리오: **Scenario X (핵심) + Scenario Y Phase 1 — Real Order Integration Skeleton & Fill Sync Readiness**.
...
## v0.16 — Real Order Integration Skeleton & Fill Sync Readiness (⏳ 진행 중)

기준선: `v0.15-final` (1693 / 201 / 23 / build). **Alembic 신규 2건** 예정 —
`0009_real_orders` (RealOrder, Phase C) + `0010_real_fills` (RealFill, Phase C).
...
W=Reconciliation — v0.17 이연)

| Phase | 작업 | 상태 | 태그 |
|---|---|---|---|
| A | RealTradingSettings 5 종 (`REAL_TRADING_ENABLED=false` / `KIS_ORDER_ENABLED=false` / `REAL_ORDER_DRY_RUN=true` / `MAX_REAL_ORDER_AMOUNT=100k` / `MAX_REAL_DAILY_ORDER_AMOUNT=1M`) + 단위 테스트 ~20 건 | ⏳ 대기 | `v0.16-real-trading-settings` |
| B | `KisOrderClientInterface` ABC + `FakeKisOrderTransport` + AST 가드 + 마스킹 단언 ~15 건 | ⏳ 대기 | `v0.16-kis-order-wrapper` |
| C | `RealOrder` ORM (40 번째) + `RealFill` ORM (41 번째) + Alembic `0009` + `0010` + Repository 2 종 + 통합 테스트 ~40 건 | ⏳ 대기 | `v0.16-real-order-orm` |
| D | `RealOrderExecutor` (dry-run 전용, 7 단계 게이트) + `FillSyncService` skeleton (mock) + 단위 ~30 + 통합 ~10 건 | ⏳ 대기 | `v0.16-real-order-executor` |
| E | 15 번째 화면 `/real-orders` (read-only, `TrendingUp` 아이콘, SafetyBanner / DryRunAttemptsTable) + `RELEASE_NOTES_v0.16.md` + 4 게이트 최종 확인 | ⏳ 대기 | `v0.16-final` |
...
> **v0.17+ 경계**: v0.16 의 `RealOrder` / `RealFill` ORM + `KisOrderClientInterface` +
> `RealOrderExecutor` 가 v0.17 의 직접 기반이 된다. v0.17 에서 Fill Sync 실 transport +
> Reconciliation 도입 후, v1.0 에서 `KIS_ORDER_ENABLED=true` + `REAL_ORDER_DRY_RUN=false`
> 를 최초로 활성화하는 **Small Approval Trading Release** 를 목표로 한다.

**v0.14 핵심 정책 (cycle-wide)**:

- **`BrokerInterface` 첫 구현체 = `SimulationBroker` 만** — 실 KIS 주문 호출 0건
- **`PAPER_TRADING_ENABLED=False` 기본** — OFF 시 전 케이스 기존 동작과 동일 (회귀 0)
- **KIS API import 0건 단언** — `SimulationBroker` 파일 내 `from app.providers.kis` 금지
- **VirtualOrder / 실 KIS 주문 물리적 분리** — `virtual_orders` 테이블 + `paper_` 접두 라우터
- **Alembic 2건 필수** — `0005_virtual_trading_core` + `0006_virtual_positions`
- **`APPROVAL` / `SMALL_AUTO` / `FULL_AUTO` 0건** — 여전히 Future Backlog
- **DART/RSS/Prometheus/Provider Data Ingestion default OFF 유지** (v0.13 정책 그대로)
- **신규 pip 의존성 0건** — 기존 의존성만

상세 계획: [`PLANS.md`](./PLANS.md) `PLAN-0014`

> **실 KIS 주문 (APPROVAL / SMALL_AUTO / FULL_AUTO)** 는 v0.14 에도
> **Future Backlog** 유지. 별도 보안·컴플라이언스·자본 한도 사이클 선행 없이는 진입 불가.
...
| Real Order Integration Skeleton (v0.16) | RealTradingSettings (5 종, paranoid defaults) + KIS Order Wrapper Skeleton (FakeKisOrderTransport) + RealOrder/RealFill ORM (40·41 번째) + RealOrderExecutor (dry-run 전용) + FillSync mock + 15 번째 화면 `/real-orders` | **v0.16 ⏳ 진행 중** — 실 KIS 주문 0건. 모든 실행은 dry-run. Reconciliation / 실 Fill Sync 는 v0.17+ |
| Fill Sync 실 연동 + Reconciliation (v0.17) | KIS 체결 조회 실 transport + RealFill 실 저장 + 내부 RealOrder vs KIS 실 포지션 비교 + mismatch 감지 + PreTradeRiskEngine 신규 룰 | v0.16 dry-run skeleton 안정 운영 후 |
| APPROVAL 모드 / v1.0 Small Approval Trading Release | `KIS_ORDER_ENABLED=true` + `REAL_ORDER_DRY_RUN=false` 최초 활성화. 사용자 승인 후 실 KIS 주문 (소액) — 비상정지 / 일일 손실 제한 / 종목당 한도 / 실거래 로그 / 보안 강화 동반 | v0.17 Reconciliation 안정 운영 후 + 컴플라이언스 / 자본 한도 / 운영자 책임 명문화 |
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v0.16 ⏳) RealTradingSettings (5 종, paranoid) + KIS Order Wrapper Skeleton (FakeKisOrderTransport) + RealOrder/RealFill ORM (40·41번째, Alembic 2건) + RealOrderExecutor (dry-run 전용) + FillSync mock + 15번째 화면 /real-orders — 실 KIS 주문 0건
   → (v0.17+) Fill Sync 실 연동 + Reconciliation + PreTradeRiskEngine 신규 룰 / ScoringEngine weight 보강 / Grafana / monitor 영속화 / 인증 고도화 / LLM 보강 / WebSocket
   → (v1.0) Small Approval Trading Release: KIS_ORDER_ENABLED=true + REAL_ORDER_DRY_RUN=false 최초 활성화 — 소액 실 KIS 주문 첫 실행 (APPROVED 후보만)
   → (Future) ReplayBroker / 스트레스 테스트 → SMALL_AUTO → FULL_AUTO
```
