...
> 본 문서는 **v0.11 마감 시점** 기준으로 갱신되었다 (마감 태그 `v0.11-final`).
> v0.11 **Real Provider Transport & Observability** 사이클 완료 —
> `HttpxDartTransport` + `HttpxRssTransport` (Phase A/B, default OFF 유지) +
> Provider Observability ring buffer + `Summary24h` + optional Prometheus
> `/metrics` (Phase C, default OFF) + `/api/health/providers` 24h aggregates +
> Settings 패널 `SuccessRateBar` / `RecentFailuresList` (Phase D). 자동매매 /
> MockBroker / FULL_AUTO 는 여전히 **Future Backlog**. Score 반영 / 백테스트
> 고도화 / 인증 고도화 / LLM 보강은 v0.12+ 후보로 연기.
...
> ring buffer + Summary24h / Prometheus exporter / `/api/health/providers` 24h
> aggregates + recent_failures + Settings 패널 보강) 은 이 목록에서 제거됨.
...
`BrokerInterface` 는 ABC placeholder 만 유지. v0.1 ~ v0.11 어디에도 구현체 없음.
...
default OFF 시 404, true 시 in-memory observability 만 노출 (mutation 0건).
...
KIS data + 휴장 캘린더 + 증권사 리포트 + 테마 매핑 + 시그널 이벤트
   ...
   → (v0.12+) DART/RSS score 반영 / 백테스트 고도화 / Grafana dashboard / 인증 고도화 / CSP / LLM 보강 / WebSocket / monitor 영속화
