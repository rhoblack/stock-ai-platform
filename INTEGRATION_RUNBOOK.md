...
> `RSS_FEED_URLS=<comma-separated>` 를 명시 설정해야 동작. **반드시 라이선스
> 검토(사람) 선행 필수** (DART OpenAPI 약관 / RSS feed 저작권). 모든 테스트는
> `respx` mock 으로 실 외부 호출 0건 유지. Prometheus exporter 도 기본 OFF —
> `PROMETHEUS_ENABLED=true` 명시 시에만 `/metrics` 200 응답. 운영자는 v0.10/v0.11
> Phase D 로 확장된 `GET /api/health/providers` 응답에서 24h aggregates +
> `recent_failures[5]` 까지 한눈에 확인할 수 있다 (read-only, POST/PUT/DELETE 모두 405).
