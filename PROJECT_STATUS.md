...
## 0. v0.8 시작 선언 — User & Migration Foundation

**v0.8 cycle 시작.** 기준선 `v0.7-final` (HEAD `1f5b01f` 시점, origin/main
...
모두 마감 위에 **User & Migration Foundation** 5 phase 진입. v0.1 ~ v0.7 동안
일관 유지된 read-only 정책의 **첫 변경 cycle** — Alembic baseline 도입 +
단일 사용자 인증 기반 + Watchlist 도메인 POST/DELETE 첫 도입.

- 시작 일자: **2026-05-06 (Asia/Seoul)**
- 시작 시점 게이트 (회귀 기준선): backend pytest **682 passed (1 deselected)**
  / frontend vitest **84 passed** / Playwright e2e **14 passed** / build 그린
- 누적 인수 태그 (예정): `v0.8-alembic-baseline` → `v0.8-auth-foundation` →
  `v0.8-watchlist-api` → `v0.8-frontend-watchlist` → `v0.8-final`
- 시작 사유 / 계획: [`PLANS.md`](./PLANS.md) `PLAN-0008`

### v0.8 핵심 목표

v0.7 마감 시점에 27 테이블 + v0.5/v0.6/v0.7 누적 ALTER 5건 → **Alembic
baseline 도입 적기**. v0.1 부터 일관 유지된 read-only 정책에 한해 **POST 라우터
첫 도입** 을 하되, 인증 (`AUTH_ENABLED` 토글 + JWT) 과 묶어서 **단일 사용자
한정** 으로 안전하게 진입한다. Watchlist 는 사용자 체감 가치가 가장 큰 단일
도메인 + 알림 / 산식 변경 0건 → POST 첫 도입의 자연스러운 후보. v0.7 의
`StrategyInterface` / `BacktestEngine` 산식 변경 0건, ScoringEngine /
HoldingCheckEngine 본 weight 변경 0건, 자동매매 / 실주문 / 실 외부 API 호출
0건 정책 모두 그대로 유지한다.
...
## 0-2. v0.7 진행 선언 — Strategy & Backtest Foundation (강등됨, Phase D 시점 스냅샷)
...
## 0-3. v0.6 마감 선언 — Fundamental & Earnings Intelligence
...
## 0-4. v0.6 진행 선언 — Fundamental & Earnings Intelligence (강등됨, Phase D 시점 스냅샷)
...
## 0-5. v0.5 마감 선언 — News, Disclosure & Theme Ranking
...
## 0-6. v0.4 마감 선언 — Analyst & Theme Intelligence
